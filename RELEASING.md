...
5. **Publish the release** to this clone's own repository (`origin`):
   ...
   ```bash
   gh release create setup-v1.1 "dist/AIPLAY Studio Setup.exe" --title "AIPLAY Studio Setup 1.1" --notes "What changed in the installer, in a sentence or two. Studio itself installs from main, so this does not need a release to update."
   ```
...
- **On the original**, it prints one line and changes nothing.
