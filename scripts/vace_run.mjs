...
/* engineIdentity now asks the app's own POST /api/engine {action:"identity"},
 * which compares the engine's launched --input-directory/--output-directory
 * against THAT APP's configured directories and hands back `problems` and
 * `matchesThisStudio` — this script no longer states its own expected
 * input/output/port, so nothing is passed here beyond BASE. */
