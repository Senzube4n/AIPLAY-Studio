...
try {
  ...
  log("\n-- a finished render is a first-class clips-library citizen --");
  /* The generation↔vfx seam: a render must land in /api/clips carrying its
   * provenance — source "vfx", the comp that made it, and its MEDIA length
   * (clipSeconds) — because that stamp is what list_clips shows and what the
   * MV workflow's import_clip reads for an honest duration. Before the stamp
   * existed a vfx render listed as source "generated" with no length. */
  if (row?.clip) {
    const genvfxClipList = await get(`/api/clips`);
    const genvfxRenderRow = (genvfxClipList.clips || []).find((c) => c.name === row.clip);
    ok("the render appears in /api/clips", !!genvfxRenderRow);
    if (genvfxRenderRow) {
      eq("...stamped source vfx", genvfxRenderRow.meta?.source, "vfx");
      eq("...pointing home at its comp", genvfxRenderRow.meta?.comp, slug);
      ok("...with its MEDIA length (the 0.3s render span, not wall time)",
        Math.abs((genvfxRenderRow.meta?.clipSeconds ?? 0) - 0.3) < 0.001,
        `clipSeconds=${genvfxRenderRow.meta?.clipSeconds}`);
    }
  }

  log("\n-- export_studio APPENDS to a display-named project instead of clobbering it --");
  /* The bug this pins: studioFile() used to append ".json" to the raw display
   * name while every save derives a slug filename — so exporting into
   * "Title — video" (the MV timeline naming shape, em dash and all) failed the
   * read silently, started from a blank project, and OVERWROTE the timeline it
   * was contracted to append to. Two exports, then an import BY DISPLAY NAME:
   * both items must be there, or the read and the write have drifted apart. */
  const GENVFX_BRIDGE_PROJ = `E2E Bridge ${stamp} — video`;
  const genvfxExport = async () => {
    const q = await api({ action: "export_studio", slug, project: GENVFX_BRIDGE_PROJ,
                          from: 0, to: 0.3, scale: 0.5, draft: true });
    let jrow = null;
    for (let i = 0; i < 240; i++) {
      const d = await get(`/api/vfx/comp/${slug}`);
      jrow = (d.renders || []).find((r) => r.id === q.jobId);
      if (jrow && (jrow.status === "done" || jrow.error)) break;
      await new Promise((r) => setTimeout(r, 500));
    }
    if (jrow?.clip) mine.add(String(jrow.clip));
    return jrow;
  };
  const genvfxExp1 = await genvfxExport();
  ok("the first export lands and reports its Studio placement",
    genvfxExp1?.status === "done" && !!genvfxExp1?.studio?.item,
    JSON.stringify(genvfxExp1?.studio ?? genvfxExp1?.error ?? null).slice(0, 140));
  const genvfxExp2 = await genvfxExport();
  ok("...so does the second", genvfxExp2?.status === "done" && !!genvfxExp2?.studio?.item,
    String(genvfxExp2?.error || genvfxExp2?.status));
  {
    const back = await api({ action: "import_studio", project: GENVFX_BRIDGE_PROJ, name: `${SLUG}-bridge` });
    made.push(back.slug);
    eq("importing BY DISPLAY NAME sees both exports — append, not clobber", back.layers, 2);
  }
  ...
  if (clipName) {
    ...
    /* The library decides what is at (100,100) — a sparse one can put a flat
     * wall of colour there, and the tracker refusing that is CORRECT (the
     * refusal is asserted above). A refusal here is an untrackable fixture,
     * not a failure — and it must not throw past every section below. */
    let t = null;
    let genvfxFlatFixture = "";
    try { t = await api({ action: "track_motion", clip: clipName, rect: [100, 100, 60, 60], toTime: 1.5 }); }
    catch (e) {
      if (!/featureless/i.test(e.message)) throw e;
      genvfxFlatFixture = e.message;
    }
    if (!t) {
      log(`  skip  ${clipName} is flat at (100,100) — real tracking not exercised (${genvfxFlatFixture.slice(0, 60)}…)`);
    } else {
