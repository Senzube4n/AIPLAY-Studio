@echo off
REM ============================================================================
REM  AIPLAY Studio — double-click this.
REM
REM  This is the whole launch story. It checks the two things that can be
REM  missing, fixes what it can, and starts the app. No terminal knowledge, no
REM  arguments, no "cd into the folder first".
REM
REM  Deliberately a .cmd and not an .exe: you can read it. A downloaded binary
REM  that wants to touch 34 GB of model weights deserves more suspicion than a
REM  fifteen-line batch file.
REM ============================================================================
setlocal
cd /d "%~dp0"
title AIPLAY Studio

echo.
echo   AIPLAY Studio
echo   -------------
echo.

REM ---- 1. Node -----------------------------------------------------------
where node >nul 2>nul
if errorlevel 1 (
  echo   Node.js is not installed, and Studio's server is written in it.
  echo.
  echo   Get the LTS installer from https://nodejs.org  ^(about 30 MB^),
  echo   run it, then double-click this file again.
  echo.
  pause
  exit /b 1
)

REM ---- 2. Dependencies ---------------------------------------------------
REM  Three packages — ws, three, gltf-validator — and all three are needed to
REM  boot. Installed on first run so the download is small.
REM  ⚠ EVERY ONE IS CHECKED, not just the first. This tested only node_modules\ws
REM  while the list was one package long; when three and gltf-validator were
REM  added, an existing install already had ws, so the check passed, npm never
REM  ran, and the server died on an import nobody had fetched.
if not exist "node_modules\ws" goto :deps
if not exist "node_modules\three" goto :deps
if not exist "node_modules\gltf-validator" goto :deps
goto :deps_ok
:deps
echo   Fetching dependencies ^(a few seconds^)...
call npm install --omit=dev --no-audit --no-fund
if errorlevel 1 (
  echo.
  echo   npm install failed. Are you online?
  pause
  exit /b 1
)
echo.
:deps_ok

REM ---- 3. Where is ComfyUI? ----------------------------------------------
REM  setup.mjs is idempotent: it exits immediately once a working engine is
REM  configured, so this costs nothing on every subsequent launch.
node scripts/setup.mjs --quiet-if-ready
if errorlevel 1 (
  echo.
  echo   Setup did not complete. Nothing was changed.
  pause
  exit /b 1
)

REM ---- 4. Go -------------------------------------------------------------
REM  The browser is opened by the server itself, the moment the port is
REM  actually accepting connections. Opening it from here raced Node's own
REM  startup and lost: the user got ERR_CONNECTION_REFUSED from a window that
REM  looked fine, which reads as a broken app rather than "wait a second".
echo   Starting. Your browser will open once the server is up.
echo   Leave this window open — closing it stops Studio.
echo.
set AIPLAY_OPEN=1
node server/index.js

REM  Only reached if the server exits. Hold the window so the error is readable
REM  rather than flashing past.
echo.
echo   AIPLAY Studio stopped.
pause
