...
## Generated assets in the compositor

One factory: everything the generation tools make is already a first-class
vfx source, and everything the compositor renders is already a first-class
generation input. No copying, no export step — the libraries ARE the seam.
Each of these flows is proven end-to-end over MCP (pixel-probed, not assumed):

- **`make_image` → layer.** The image lands in the images library and its
  name is a legal `vfx_add_layer type:"image"` src immediately.
- **The editor is the same library.** `image_cutout` (BiRefNet) and
  `image_adjust` write NEW files beside the original — the cutout's RGBA
  survives the whole chain, so a knocked-out subject composites over a solid
  with real transparency, and an adjusted image is a vfx src the moment the
  call returns. The comp and the editor read the same file; the original is
  never touched.
- **`make_clip` → video layer.** An LTX clip is a `type:"video"` src by name;
  trim it (`in_point`), scale it, stack effects, render — the movie carries
  the generated footage (and its audio, when it has any).
- **`vfx_render` → the library, with provenance.** An mp4/mov render lands in
  the clips library stamped `source: "vfx"`, its comp slug, and its MEDIA
  length — `list_clips` shows it like any generated clip, and `restyle_clip`
  will happily take it as a source (the polish-then-regenerate loop).
- **The MV return trip.** `mv_build_timeline` → `vfx_import_studio` pulls the
  cut into a comp; polish it; `vfx_render`; then `mv_import_clip` makes the
  render a TAKE on the scene — `mv_build_timeline` cuts it in and
  `mv_read_timeline` owns it (`in_sync: true`, not a foreign item). Switch
  between the generated and the polished take with `mv_pick_take
  target:"clip"`. `vfx_export_studio` is the other door: it appends its render
  to a named Studio timeline as an overlay item (with a `vfxComp` pointer
  home), which the MV reader reports honestly as an item the workflow never
  made.
