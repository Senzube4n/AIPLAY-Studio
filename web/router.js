...
function fieldHtml(f) {
  ...
  if (f.type === "media" && (f.media || "image") === "image") {
    ...
    ctl = `<input id="${id}" class="in2 num" type="number" data-f="${esc(f.name)}" data-t="${f.type}"${f.min != null ? ` min="${f.min}"` : ""}${f.max != null ? ` max="${f.max}"` : ""}${f.type === "number" ? ' step="any"' : ""}${f.simple && f.default != null ? ` value="${esc(f.default)}"` : ""} placeholder="${esc(f.default ?? "")}">`;
  } else if (f.type === "json") {
