...
function ownState() {
  return {
    ...
    karaoke: S.karaoke, showTitle: S.showTitle,
...
function projectDocument() {
  return { ...(S.docExtra || {}), v: 1, mvProjectId: S.mvProjectId ?? null, ...ownState() };
}

/** History owns its values; later slider edits must not mutate an older entry. */
function historySnapshot() {
  return JSON.parse(JSON.stringify(ownState()));
}

/** One edit per slider gesture, with recovery refreshed through its last input. */
let settingsGesture = null;
function editProjectSettings(change, gesture = null) {
  if (!gesture || settingsGesture !== gesture) pushUndo();
  settingsGesture = gesture;
  change();
  autosave();
}

function bindProjectInput(id, change) {
  const el = $(id);
  if (el.type === "range") {
    el.oninput = () => editProjectSettings(change, id);
    el.onchange = el.onblur = () => { settingsGesture = null; };
  } else {
    el.onchange = () => editProjectSettings(change);
  }
}
...
function autosave() {
  ...
  saveTimer = setTimeout(() => {
    try {
      ...
      /* FORK: docExtra rides along too. The named save preserves foreign keys by
       * spreading what was opened; CRASH RECOVERY has to do the same or an
       * MV-composed timeline comes back without its mvProjectId and every
       * regenerate button on it stops working — the exact failure the fork
       * patched once in projDoc(), reappearing one function to the left. */
      localStorage.setItem(SAVE_KEY, JSON.stringify({ ...projectDocument(), at: Date.now() }));
...
async function restoreAutosave() {
  ...
  await restoreProjectState(d, true);
  $("stProjName").textContent = d.name || "Untitled";
  // Recovery can contain edits newer than the named file on disk.
  $("stProjName").classList.add("dirty");
  return true;
}

/** Named open, crash recovery and undo restore the same editable document. */
async function restoreProjectState(d, newProject = false) {
  settingsGesture = null;
  if (newProject) {
    S.docExtra = { ...d };
    S.mvProjectId = d.mvProjectId ?? null;
    S.undo = []; S.redo = [];
  }
  S.fx = { ...S.fx, ...d.fx };
  S.vis = d.vis ?? S.vis;
  S.out = { ...S.out, ...d.out };
  S.karaoke = typeof d.karaoke === "boolean" ? d.karaoke : true;
  S.showTitle = typeof d.showTitle === "boolean" ? d.showTitle : true;
  ...
  if (Number.isFinite(d.laneH)) S.laneH = d.laneH;
  writeRenderOpts();
  S.beats = null;
...
async function undo() {
  ...
  await restoreProjectState(S.undo.pop());
  window.dispatchEvent(new CustomEvent("aiplay-studio-edited"));
  autosave();
}
...
async function redo() {
  ...
  S.undo.push(historySnapshot());
  await restoreProjectState(S.redo.pop());
  window.dispatchEvent(new CustomEvent("aiplay-studio-edited"));
  autosave();
}
...
function render() {
  ...
  /* Set when some item's time window covers `t`, even if it could not be drawn.
   * See the hold-the-last-frame fallback after the layer loop. */
  let wantedPicture = false;
  ...
  for (const tr of vts) {
    ...
    for (const it of [...tr.items].sort((a, b) => a.start - b.start)) {
      ...
      /* This item OWNS this instant, whether or not it can be painted yet. The
       * distinction matters below: a clip that is late to decode must not look
       * the same as a stretch of timeline with nothing on it. */
      wantedPicture = true;
  ...
  /* HOLD THE LAST FRAME rather than flashing black at a cut.
   *
   * The readyState guard above skips a clip that has not decoded yet, and every
   * frame begins with a black fillRect — so at a cut, where the outgoing item's
   * window has just ended and the incoming element is still opening, the frame
   * that reaches the recorder is BLACK. On a 24-cut timeline that is 24 chances
   * to blink, and it is visible in the exported file.
   *
   * Redrawing the previous element is close to free: it still holds its last
   * decoded frame, so this is one more drawCover rather than a canvas copy,
   * which matters when the export is already struggling to hit its frame rate.
   *
   * Gated on `wantedPicture`, so a genuine hole in the coverage still reads as
   * black. That is honest — the timeline really has nothing there, and hiding it
   * would make a missing clip look like a held shot. */
  if (!drewAnything && wantedPicture && S.lastPainted
      && (S.lastPainted.complete || S.lastPainted.readyState >= 2)) {
    drawCover(ctx, S.lastPainted, W, H);
    drewAnything = true;
  }
  ...
  if (vig > 0) {
    /* PRE-RENDERED, not filled per frame.
     *
     * This built a radial gradient and filled the whole canvas with it on every
     * single frame. A gradient fill over 2.1 megapixels is one of the slowest
     * things canvas2d does, and the export is real-time: whatever a frame costs
     * comes straight off the frame RATE. Measured before this change, a
     * 1920x1088 export sustained 16.9 fps against a requested 24.
     *
     * The vignette only depends on the size and the amount, so it is drawn once
     * into an offscreen canvas and blitted afterwards. Keyed on all three, so a
     * resize or a change of amount rebuilds it exactly once. */
    const key = `${W}x${H}x${vig.toFixed(3)}`;
    if (S.vigKey !== key) {
      const off = document.createElement("canvas");
      off.width = W; off.height = H;
      const og = off.getContext("2d");
      const g = og.createRadialGradient(W / 2, H / 2, Math.min(W, H) * 0.32, W / 2, H / 2, Math.max(W, H) * 0.72);
...
/**
 * The inverse of readRenderOpts, and the reason it has to exist.
 *
 * Loading a project sets S.out but left the render controls showing whatever
 * they showed before — and startExport() BEGINS with readRenderOpts(), which
 * reads those controls straight back over S.out. So a project saved at
 * 1920x1080/24 exported at whatever the selects happened to say, silently,
 * while the status bar displayed the correct numbers the whole time. Measured:
 * a freshly opened 1920x1080/24/16 timeline had stResW="1280x720", stFps="30",
 * stMbps="8" — every one of them the default, none of them the project.
 *
 * A size or rate with no matching option gets one APPENDED rather than being
 * allowed to snap silently: a select whose value matches no option reports "",
 * and readRenderOpts falls back to 1280x720 on exactly that — which is the same
 * bug wearing a different hat.
 *
 * Ends by calling readRenderOpts() so the canvas resize and the size estimate
 * still happen in the one place that owns them.
 */
function writeRenderOpts() {
  const pick = (el, value, label) => {
    if (!el) return;
    const v = String(value);
    /* Drop any option THIS function added on a previous load. Without it every
     * project with an off-list size leaves another entry behind, and the menu
     * grows one stale "· project" line per open while only one of them is the
     * project actually loaded. */
    for (const o of [...el.options]) {
      if (o.dataset.appended && o.value !== v) o.remove();
    }
    if (![...el.options].some((o) => o.value === v)) {
      const o = document.createElement("option");
      o.value = v; o.textContent = label; o.dataset.appended = "1";
      el.appendChild(o);
    }
    el.value = v;
  };
  pick($("stResW"), `${S.out.w}x${S.out.h}`, `${S.out.w} × ${S.out.h} · project`);
  pick($("stFps"), S.out.fps, `${S.out.fps}`);
  if ($("stCodec") && S.out.codec) $("stCodec").value = S.out.codec;
  if ($("stMbps")) $("stMbps").value = String(S.out.mbps);
  readRenderOpts();
}
...
async function startExport() {
  ...
  /* A WALL-CLOCK DEADLINE, because the playback guard cannot be trusted to end
   * this and a frame count is the wrong unit.
   *
   * tick() already stops the export when the playhead reaches the end, and that
   * test was itself a fix for an export running past the song. It is not enough:
   * it reads the playhead, the playhead follows the audio element, and an element
   * that loops or restarts never reports a time at or past `total`. Measured on a
   * 44-second project: 1556 frames captured, 64.8 s of wall clock, still going.
   *
   * ⚠ Counting FRAMES instead was tried and is wrong, which is worth recording
   * because it looks right. MediaRecorder captures the audio track in REAL TIME
   * while this timer feeds the video track, so the two only agree if the render
   * actually sustains `fps`. It does not: 1055 frames at 1920x1088 took 63.7 s,
   * an effective 16.6 fps, and the file came out with 44 s of video against 63.7 s
   * of audio — in sync with nothing.
   *
   * Wall clock is the honest authority for a real-time capture: it is exactly
   * what the audio track will contain, so ending on it keeps the two the same
   * length. A slow render then costs frame RATE, which is visible and honest,
   * rather than desync, which is not. */
  const deadlineMs = Math.max(100, (totalLength() - S.t) * 1000);
  ...
  S.recTimer = setInterval(() => {
    ...
    track.requestFrame?.();
    if (performance.now() - startedAt >= deadlineMs) stopExport();
  }, 1000 / S.out.fps);
...
export function initStudio() {
  ...
  if (!S.tracks.length) {
    ...
    if (hasSave) {
      // Let the controls finish wiring before recovery synchronises their values.
      Promise.resolve().then(restoreAutosave).then((had) => {
  ...
  bindProjectInput("stVis", () => { S.vis = $("stVis").value; render(); });
  bindProjectInput("stKaraoke", () => { S.karaoke = $("stKaraoke").checked; render(); });
  bindProjectInput("stTitleCard", () => { S.showTitle = $("stTitleCard").checked; render(); });
  ...
  if ($("stLaneH")) { $("stLaneH").value = S.laneH; bindProjectInput("stLaneH", laneRead); laneRead(); }
  ...
  for (const id of ["stBeatSens", "stBeatBand", "stBeatDrive", "stBeatSmooth"]) bindProjectInput(id, beatRead);
  beatRead();
  ...
  window.addEventListener("aiplay-studio-beatcfg", () => {
    beatShow(); fxSync();
    $("stVis").value = S.vis;
    $("stKaraoke").checked = S.karaoke;
    $("stTitleCard").checked = S.showTitle;
    $("stVisSize").value = Math.round(S.visSize * 100);
    $("stVisOpacity").value = Math.round(S.visOpacity * 100);
    visRead();
    if ($("stLaneH")) { $("stLaneH").value = S.laneH; laneRead(); }
  });
  ...
  bindProjectInput("stVisSize", visRead);
  bindProjectInput("stVisOpacity", visRead);
  visRead();
  ...
  for (const id of ["stResW", "stFps", "stMbps", "stCodec"]) bindProjectInput(id, readRenderOpts);
  readRenderOpts();
  $("stClear").onclick = () => {
    pushUndo();
  ...
  /* FORK: mvProjectId is named explicitly as well as spread. The spread covers
   * a project that was OPENED, but a timeline composed fresh from the workflow
   * sets S.mvProjectId at runtime with no opened doc behind it — so relying on
   * docExtra alone would lose the linkage on exactly the projects the bridge
   * exists for. */
  const projDoc = projectDocument;
  ...
  $("stSave").onclick = async () => {
    ...
    setProjName(r.name);
    S.docExtra = { ...projDoc(), name: r.name };
  ...
  $("projList").addEventListener("click", async (e) => {
    ...
    if (r.error) { toast(r.error); return; }
    const d = r.doc || {};
    await restoreProjectState({ ...d, t: 0 }, true);
...
/* ─────────────── Video Workflow linkage (FORK — see FORK_DELTA.md) ───────────
 *
 * A timeline composed by the workflow stamps `mvClipId` on every item and
 * `mvProjectId` on the doc. That metadata is the whole contract: it is what
 * lets a right-click say "regenerate this shot" and have the server re-roll
 * the SAME scene — same segment, same cast, same soundtrack window — under a
 * fresh seed, because the relationships live in the project document rather
 * than in anyone's memory.
 *
 * The menu is delegated and built on demand, because #stLanes is rebuilt by
 * every paintTimeline() and anything held by reference would go stale.
 */
(() => {
  let menu = null;
  ...
  document.addEventListener("contextmenu", (e) => {
    const el = e.target.closest?.("#stLanes [data-item]");
    if (!el) return;
    const tr = S.tracks.find((x) => x.id === +el.dataset.track);
    const it = tr?.items.find((x) => x.id === +el.dataset.item);
    if (!it) return;
    e.preventDefault();
    closeMenu();

    menu = document.createElement("div");
    menu.className = "stctx";
    const rows = [];
    if (it.mvClipId && S.mvProjectId) {
      rows.push(`<button data-act="regen">↻ Regenerate scene (new seed)</button>`);
      rows.push(`<button data-act="takes">≡ Takes…</button>`);
    }
    rows.push(`<button data-act="split">✂ Split at playhead</button>`);
    rows.push(`<button data-act="remove">✕ Remove</button>`);
    ...
    menu.addEventListener("click", async (ev) => {
      ...
      if (act === "split") { S.sel = it.id; seek(S.t); splitAtPlayhead(); return; }
      if (act === "remove") {
        pushUndo();
        tr.items = tr.items.filter((x) => x.id !== it.id);
        paintTimeline(); render();
        return;
      }

      if (act === "regen") {
        /* The render takes minutes; the item shows it is working and the
         * result swaps in when it lands. The OLD take is never destroyed —
         * it stays in the workflow project's takes list. */
        toast("Regenerating this scene — the new take swaps in when it lands…");
        try {
          const r = await (await fetch("/api/mv", {
            method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ action: "regen_by_clip_id", slug: S.mvProjectId, clipId: it.mvClipId }),
          })).json();
          if (r.error) throw new Error(r.error);
          pushUndo();
          it.src = `/api/clip/${encodeURIComponent(r.clip)}`;
          await attach(it, "video", { keepTiming: true });
          paintTimeline(); render();
          toast(`New take in place (${(r.takes || []).length} takes kept)`);
        } catch (err) { toast(String(err.message || err)); }
        return;
      }

      if (act === "takes") {
        /* Cycle through the kept takes in place — the fastest possible A/B:
         * right-click, takes, watch, repeat. */
        try {
          const d = await (await fetch(`/api/mv/project/${encodeURIComponent(S.mvProjectId)}`)).json();
          const row = d.project?.clips?.find((c) => c.id === it.mvClipId);
          const takes = row?.takes || [];
          if (takes.length < 2) { toast("Only one take so far — Regenerate makes more."); return; }
          const cur = decodeURIComponent(it.src.split("/").pop());
          const at = takes.findIndex((t) => t.clip === cur);
          const next = takes[(at + 1) % takes.length];
          pushUndo();
          it.src = `/api/clip/${encodeURIComponent(next.clip)}`;
          await attach(it, "video", { keepTiming: true });
          paintTimeline(); render();
          toast(`Take ${((at + 1) % takes.length) + 1}/${takes.length} (seed ${next.seed})`);
        } catch (err) { toast(String(err.message || err)); }
      }
    });
  });
