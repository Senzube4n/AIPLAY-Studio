...
/**
 * THE ONE CROSSING. Everything else in this file talks to /api/daw/ear, which
 * is why server/daw/ui_test.js's parity census reads daw.js and voicelab.js
 * and deliberately not this module. The reference LIBRARY does not live behind
 * that door — it is `profile_list` on /api/daw, the same action
 * daw_profile_list and the Voice Lab's picker post — so this helper exists,
 * alone, and is used for exactly that one read.
 *
 * It is a read. Nothing in this file WRITES through /api/daw, and the chosen
 * profile reaches the server as one field on the `critique` body the Ear
 * already posts — never as a second call that could disagree with it.
 * server/daw/voiceui_test.js holds this to one action.
 */
async function postDaw(body) {
...
export function mountEar(opts = {}) {
  ...
  const S = {
    ...
    /* the reference profile this critique is measured against, if any —
     * an id, never a copy of the profile itself */
    profiles: null, profileId: "", profilesAsked: false,
  };
  ...
  function toggle(on) {
    ...
    if (S.open && !S.profilesAsked) { S.profilesAsked = true; loadProfiles(); }
    if (S.open) render();
  }
  ...
  /* ── the reference row ────────────────────────────────────────────────
   * A profile is a SHAPE — dB shares, dB off the pink null, decay times in
   * ms, counts — measured off a record somebody owns. Never audio, never a
   * melody. Picking one here makes the critique measure this mix against
   * that shape as well as against the pink null, and the cards it produces
   * are ordinary cards with ordinary routes.
   *
   * The list is read once per session and never held as more than ids; the
   * profile itself is only ever read by the server that measures with it.
   * A machine with no library simply has no row. */
  async function loadProfiles() {
    try {
      const r = await postDaw({ action: "profile_list" });
      S.profiles = Array.isArray(r.profiles) ? r.profiles : [];
    } catch { S.profiles = null; }
    drawRefSel();
    render();
  }
  ...
  async function listen() {
    ...
    try {
      /* key and target ride only when typed: absent, the Ear reads the root
       * off the project (or infers it and says so) and takes the genre's
       * loudness target (edm: -8 club / -14 streaming). */
      const tl = Number(targetBox.value);
      /* the reference profile rides the SAME body, as one id, so a critique
       * and the shape it was measured against can never come apart */
      const r = await post({ action: "critique", slug, genre: genreSel.value,
                             ...(keyBox.value.trim() ? { key: keyBox.value.trim() } : {}),
                             ...(targetBox.value.trim() && Number.isFinite(tl) ? { target_lufs: tl } : {}),
  ...
  /* The two facts the Ear would otherwise have to infer: the song's root
   * (for the kick-tuning critic) and the loudness target (for the
   * loudness_target card). Same fields daw_critique takes over MCP. */
  const keyBox = EL("input");
  keyBox.placeholder = "key (F, Db…)";
  keyBox.size = 9;
  keyBox.title = "the song's root, for the kick-tuning critic; blank = read off the project or inferred from the bass line (and said so)";
  const targetBox = EL("input");
  targetBox.placeholder = "target LUFS";
  targetBox.size = 10;
  targetBox.inputMode = "decimal";
  targetBox.title = "integrated LUFS to aim at; blank = the genre's (edm: -8 club) or the -14 streaming default";
  /* THE REFERENCE ROW. One select. `none` is the default and is the state
   * every critique before this one ran in, so picking nothing changes
   * nothing. */
  const refSel = EL("select");
  refSel.title = "measure this mix against a reference profile as well as against the pink "
    + "null. A profile is SHAPE only — band levels, decay times, sidechain depth and "
    + "recovery, counts — measured off a record you own. It carries no audio and no melody, "
    + "so this is a style target and never a copy.";
  refSel.addEventListener("change", () => { S.profileId = refSel.value; render(); });
  function drawRefSel() {
    const keep = S.profileId;
    refSel.textContent = "";
    const none = EL("option", null, "no reference");
    none.value = "";
    ...
    refSel.value = keep && (S.profiles || []).some((p) => p.id === keep) ? keep : "";
    S.profileId = refSel.value;
  }
  drawRefSel();
  ...
  function renderListen() {
    const row = EL("div", "ear-row");
    row.append(listenBtn, genreSel, keyBox, targetBox);
    body.append(row);

    /* THE REFERENCE ROW, and it says what it is either way. With profiles on
     * the machine it is a picker plus one sentence about what a profile is
     * and is not; with none it is one sentence naming where they come from,
     * so the absence is an answer rather than a missing feature. Both use the
     * classes this stylesheet already has — nothing new to style. */
    if (S.profiles?.length) {
      const rrow = EL("div", "ear-row");
      rrow.append(EL("label", null, "reference"), refSel);
      body.append(rrow);
    }
    body.append(EL("div", "ear-note",
      S.profiles === null
        ? "No reference library on this machine — the critique measures against the pink "
          + "null alone, which is what it has always done."
        : !S.profiles.length
          ? "No reference profiles yet. Build one in the Voice Lab's reference bar (or with "
            + "daw_profile_build) and it appears here: shape measurements taken off a record "
            + "you own — band levels, decay times, sidechain depth and recovery — never its "
            + "audio and never its melody."
          : S.profileId
            ? "This critique is measured against that profile's SHAPE as well as against the "
              + "pink null. A profile is dB numbers, times in ms and counts; no audio and no "
              + "melody is copied out of a reference, so this is a style target, not a copy."
            : "Pick one and the critique measures this mix against that record's shape as "
              + "well as against the pink null."));
