...
let runtime=null,appearance=null;
...
if(overlayMode)document.body.classList.add('avatar-overlay');
...
function initViewer(){
  ...
  new ResizeObserver(()=>{const w=viewport.clientWidth,h=viewport.clientHeight;renderer.setSize(w,h);camera.aspect=w/h;camera.updateProjectionMatrix();if(model)cameraView('fit');}).observe(viewport);
  let last=performance.now();renderer.setAnimationLoop(now=>{const dt=Math.min((now-last)/1000,.05);last=now;if(document.hidden)return;if(playing&&mixer){mixer.update(dt*Number($('speed').value));updateTime();}runtime?.update(dt);controls.update();renderer.render(scene,camera);});
}
...
async function select(id){
  ...
  try{
    ...
    model=gltf.scene;runtime=createAvatarRuntime(gltf);scene.add(model);model.updateMatrixWorld(true);clips=gltf.animations;mixer=new THREE.AnimationMixer(model);helper=new THREE.SkeletonHelper(model);helper.visible=$('skeleton').checked;scene.add(helper);
    ...
    $('wireframe').dispatchEvent(new Event('change'));$('overlay-open').href=`/avatars.html?id=${id}&overlay=1`;history.replaceState(null,'',`?id=${id}${overlayMode?'&overlay=1':''}`);status(row.inspection.profile==='vrm'?'Local VRM ready.':'Skin verified. Review deformation before handoff.');
