...
export function mountAvatarWardrobeUI({row, gltf, load, isCurrent = () => true,
  ...
  return {setLook, importPart, snapshot:()=>({selection:selection?structuredClone(selection):null,dirty,busy:busy||previewing,previewValid}),update:() => preview.update(), dispose() {live = false; context++; clearTimer(timer); preview.dispose();}};
}
