 ...
 * vfx_* is stage 4, POLISH: seconds per frame against the generators'
 * minutes per clip. Reach for it for titles/text/logos (vfx_templates
 * first), grades and looks (vfx_effect_presets makes one reusable),
 * compositing generated elements, transitions, particle/light moments, and
 * renders that must carry the audio mix (vfx_render muxes it; audio layers
 * come from the music library). It does NOT replace generation for new
 * photography — a different subject or camera is make_clip's job. The seam
 * to stage 5: vfx_import_studio pulls a Studio cut in, vfx_export_studio
 * drops a finished comp back onto the timeline.
 */
  ...
  const tools = [
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          layer_id: { type: "string", description: "Layer id or unambiguous name — an audio or video layer." },
          profile: { type: "string", enum: ["guitar", "bass", "piano"], description: "Which stem this is. 'guitar' default. 'piano' hears the full 27.5-4186 Hz keyboard with a gentle gate — its thresholds are reasoned rather than swept, so check the result." },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          profile: { type: "string", enum: ["guitar", "bass", "piano"], description: "Stem profile for the transcription. 'guitar' default; use 'piano' for the piano rig." },
...

    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          /* ⚠ THESE FOUR WERE MISSING, and `additionalProperties: false` made
           * that a refusal rather than an omission: the route has always read
           * b.camera, b.width, b.height and b.animators, so an agent passing
           * them got them silently dropped — or rejected before the call left.
           *
           * The camera one has teeth. engine.py reads pointOfInterest, and
           * without it camera_from leaves the rotation at identity — a camera
           * that MOVES and is never AIMED. Which is exactly what a flying-bullet
           * intro looks like when it "fails", and is the likeliest origin of the
           * handover's note about a vfx_add_layer error. The return shape was
           * never the problem; this is. */
          camera: {
            type: "object",
            description:
              "camera layers: zoom, focalLength, depthOfField, aperture, blurLevel, and "
              + "pointOfInterest [x,y,z] in comp pixels. ⚠ WITHOUT pointOfInterest the "
              + "camera is placed but never aimed — it will move and look at nothing.",
            additionalProperties: true,
          },
          width: { type: "integer", description: "solid layers: width in pixels. Defaults to the comp's." },
          height: { type: "integer", description: "solid layers: height in pixels. Defaults to the comp's." },
          animators: {
            type: "array",
            description: "text layers: per-character animators, the AE text-animator model.",
      ...
      async run(a) {
        const r = await vfx({
          ...
          camera: a.camera, width: a.width, height: a.height, animators: a.animators,
...

    {
      ...
      description:
        ...
        + "Studio timeline and every clip tool can see them — stamped with their provenance "
        + "(source \"vfx\", the comp slug, the MEDIA length), so list_clips shows what made "
        + "them and how long they really are.\n"
