...
console.log("\n§2  the editor, the library, the route and the finish");
{
  ...
  ok("the finish hands back to the original at replaceTo", /library\.replaceSection\(job\.extendedFrom, h\.file, at, job\.replaceTo, \{ from: isYueExt \? at : 0 \}\)/.test(finish));
  ok("...remembers where, and files it as a mix with no trajectory and no run folder",
    /replacedTo: job\.replaceTo,/.test(finish) && !/\bcodes:/.test(finish) && !/\byueDir:/.test(finish) && /return;\n\s+\}\n\s*$/.test(finish));
