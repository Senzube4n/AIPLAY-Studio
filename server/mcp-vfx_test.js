...
ok("vfx_set_comp forwards seed", sourceOf(setComp.run).includes("seed: a.seed"));

const setProp = tools.find((t) => t.name === "vfx_set_property");
ok("vfx_set_property forwards expr", sourceOf(setProp.run).includes("expr: a.expr"));

const setLayer = tools.find((t) => t.name === "vfx_set_layer");
for (const f of ["threeD", "camera", "shapes", "animators", "collapse", "frameBlend", "styles",
                 "shy", "label"]) {
  ok(`vfx_set_layer forwards ${f}`, sourceOf(setLayer.run).includes(f));
}

ok("vfx_set_comp forwards hide_shy",
  sourceOf(setComp.run).includes("hideShy: a.hide_shy"));
...
for (const act of ["list_fx_presets", "save_fx_preset", "apply_fx_preset",
  ...
  ok(`...and posts ${act}`, sourceOf(fxp?.run || "").includes(`"${act}"`));
}
ok("...forwarding include_transform on the wire spelling",
  sourceOf(fxp?.run || "").includes("includeTransform: a.include_transform"));
ok("...and every op in the schema has a branch",
  (fxp?.inputSchema.properties.op.enum || []).every((op) => sourceOf(fxp.run).includes(`case "${op}"`)));
...
ok("...and forwards it", sourceOf(preview.run).includes(`q.set("view"`));

const probe = tools.find((t) => t.name === "vfx_probe_pixel");
ok("vfx_probe_pixel exists", !!probe);
ok("...and posts the probe_pixel action", sourceOf(probe?.run || "").includes(`"probe_pixel"`));

const align = tools.find((t) => t.name === "vfx_align_layers");
ok("vfx_align_layers exists", !!align);
ok("...and posts the align_layers action", sourceOf(align?.run || "").includes(`"align_layers"`));

const overlayTool = tools.find((t) => t.name === "vfx_view_overlay");
ok("vfx_view_overlay exists", !!overlayTool);
ok("...and posts the view_overlay action", sourceOf(overlayTool?.run || "").includes(`"view_overlay"`));

const status = tools.find((t) => t.name === "vfx_render_status");
ok("vfx_render_status exists", !!status);
ok("...and reads /api/vfx/renders", sourceOf(status?.run || "").includes("/api/vfx/renders"));
...
ok("vfx_set_layer forwards the audio switch", sourceOf(setLayer.run).includes("audio: a.audio"));
ok("vfx_set_layer forwards audioLevels", sourceOf(setLayer.run).includes("audioLevels: a.audio_levels"));
...
ok("vfx_set_layer forwards autoOrient", sourceOf(setLayer.run).includes("autoOrient: a.auto_orient"));
...
ok("vfx_import_studio forwards audio_as", sourceOf(importStudio.run).includes("audioAs: a.audio_as"));
...
ok("...and posts the set_guides action", sourceOf(guidesTool?.run || "").includes(`"set_guides"`));
...
ok("...and posts the precompose action", sourceOf(pcpTool?.run || "").includes(`"precompose"`));
ok("...and hands back the comp layer's id, not a placeholder",
  sourceOf(pcpTool?.run || "").includes("comp_layer_id"));
ok("...and surfaces the boundary warnings", sourceOf(pcpTool?.run || "").includes("warnings: r.warnings"));
...
ok("...posts the audio_peaks action", sourceOf(peaksTool?.run || "").includes(`"audio_peaks"`));
ok("...forwards layer_id as layerId", sourceOf(peaksTool?.run || "").includes("layerId: a.layer_id"));
ok("...forwards pixels_per_second as pixelsPerSecond",
  sourceOf(peaksTool?.run || "").includes("pixelsPerSecond: a.pixels_per_second"));
ok("...forwards src and bins", sourceOf(peaksTool?.run || "").includes("src: a.src")
  && sourceOf(peaksTool?.run || "").includes("bins: a.bins"));
...
ok("...posts the audio_notes action", sourceOf(notesTool?.run || "").includes(`"audio_notes"`));
ok("...forwards layer_id as layerId", sourceOf(notesTool?.run || "").includes("layerId: a.layer_id"));
ok("...forwards the profile, the fingering switch, tuning and frets",
  ["profile: a.profile", "fingering: a.fingering", "tuning: a.tuning", "frets: a.frets"]
    .every((s) => sourceOf(notesTool?.run || "").includes(s)));
/* ⚠ THIS USED TO BE A HARDCODED ["guitar", "bass"], and it caught a real thing
 * when a piano profile was added — so the guard is kept, but pointed at what it
 * was actually protecting.
 *
 * Two separate rules, because they fail for different reasons:
 *   PARITY   the enum must be exactly the profiles notes.py defines. A profile
 *            offered but not implemented is a crash; one implemented but not
 *            offered is unreachable. A literal list could not see either.
 *   HONESTY  guitar and bass carry an F1 from a four-piece sweep. piano does
 *            not — its thresholds are reasoned. Offering it is fine; offering
 *            it without saying so is not, so the description has to admit it. */
const PROFILE_SRC = readFileSync(new URL("./vfx/notes.py", import.meta.url), "utf8");
const implemented = [...PROFILE_SRC.matchAll(/^\s{4}"([a-z]+)":\s*dict\(/gm)].map((m) => m[1]);
const offered = notesTool?.inputSchema?.properties?.profile?.enum || [];
/* THREE copies of this list existed and they disagreed. The HTTP validator in
 * vfx/routes.js is the one that actually refuses a request, so a profile missing
 * from THERE is a server rejecting something it can do.
 *
 * ⚠ Split on a character code, not on an escape. Two earlier attempts wrote
 * this with "\n" in the source and the escape did not survive being written to
 * the file, so the string literal broke across two lines and the module would
 * not parse. Same family as the backspace-for-word-boundary trap this repo has
 * hit before: when a pattern needs an escape to be correct, prefer the version
 * that needs none. */
const ROUTES_SRC = readFileSync(new URL("./vfx/routes.js", import.meta.url), "utf8");
const NL = String.fromCharCode(10);
const profBlockAt = ROUTES_SRC.indexOf("const NOTE_PROFILES = {");
const profBlock = profBlockAt < 0 ? "" : ROUTES_SRC.slice(profBlockAt, ROUTES_SRC.indexOf("};", profBlockAt));
const validated = profBlock.split(NL).slice(1)
  .map((ln) => ln.trim())
  .filter((ln) => ln.includes(":"))
  .map((ln) => ln.slice(0, ln.indexOf(":")))
  .filter((w) => w.length > 0 && w === w.toLowerCase() && /^[a-z]+$/.test(w));
ok("...and the HTTP validator accepts exactly the same set",
  validated.length > 0
    && JSON.stringify([...validated].sort()) === JSON.stringify([...implemented].sort()),
  `validated ${JSON.stringify(validated)} vs implemented ${JSON.stringify(implemented)}`);
ok("...offers exactly the profiles notes.py implements",
  JSON.stringify([...offered].sort()) === JSON.stringify([...implemented].sort()),
  `offered ${JSON.stringify(offered)} vs implemented ${JSON.stringify(implemented)}`);
ok("...and any profile without a measured sweep says so where a caller will see it",
  !offered.includes("piano")
    || /reasoned rather than swept|check the result/i.test(
         notesTool?.inputSchema?.properties?.profile?.description || ""),
  "piano's thresholds carry no F1 — the description must not imply they do");
ok("...and refuses parameters it does not know (additionalProperties:false)",
  notesTool?.inputSchema?.additionalProperties === false);
ok("...its reply surfaces the cache claim (`cached`), the peaks idiom",
  sourceOf(notesTool?.run || "").includes("cached: r.cached"));

const rigTool = tools.find((t) => t.name === "vfx_instrument_rig");
ok("vfx_instrument_rig exists", !!rigTool);
ok("...posts the instrument_rig action", sourceOf(rigTool?.run || "").includes(`"instrument_rig"`));
ok("...forwards left_handed as leftHanded", sourceOf(rigTool?.run || "").includes("leftHanded: a.left_handed"));
ok("...forwards bend_visual as bendVisual", sourceOf(rigTool?.run || "").includes("bendVisual: a.bend_visual"));
ok("...forwards notes, audio, profile, frets, tab, roll, tuning, colors, name",
  ["notes: a.notes", "audio: a.audio", "profile: a.profile", "frets: a.frets",
   "tab: a.tab", "roll: a.roll", "tuning: a.tuning", "colors: a.colors", "name: a.name"]
    .every((s) => sourceOf(rigTool?.run || "").includes(s)));
...
ok("vfx_add_layer offers the light layer kind",
  ...
  JSON.stringify(addLayer.inputSchema.properties.type.enum));
ok("vfx_add_layer forwards the light spec", sourceOf(addLayer.run).includes("light: a.light"));
ok("vfx_set_layer forwards light", sourceOf(setLayer.run).includes("light: a.light"));
ok("vfx_set_layer forwards material", sourceOf(setLayer.run).includes("material: a.material"));
...
ok("...and posts the set_mask action", sourceOf(setMask?.run || "").includes(`"set_mask"`));
ok("...forwarding mask_id as maskId", sourceOf(setMask?.run || "").includes("maskId: a.mask_id"));
...
ok("...and posts the remove_mask action", sourceOf(rmMask?.run || "").includes(`"remove_mask"`));
ok("...forwarding mask_id as maskId", sourceOf(rmMask?.run || "").includes("maskId: a.mask_id"));

console.log("\n  -- the RAM preview is drivable from MCP --");

const prewarm = tools.find((t) => t.name === "vfx_prewarm");
ok("vfx_prewarm exists", !!prewarm);
ok("...and posts the prewarm action", sourceOf(prewarm?.run || "").includes(`"prewarm"`));
ok("...and the cancel op posts prewarm_cancel", sourceOf(prewarm?.run || "").includes(`"prewarm_cancel"`));
ok("...forwarding job_id as jobId", sourceOf(prewarm?.run || "").includes("jobId: a.job_id"));
...
ok("...and its run surfaces `cleared`", sourceOf(setProp.run).includes("cleared"));

console.log("\n  -- align says when it could not move something --");

ok("vfx_align_layers surfaces the plane-bounds warnings",
  sourceOf(align.run).includes("warnings: r.warnings"));
