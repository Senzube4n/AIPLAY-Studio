  ...
  return [
    {
      ...
      description:
        ...
        + "confirmation dialog. Three critics read the things a big-room drop "
        + "shipped wrong: `loudness_target` (integrated LUFS against the project's "
        + "target_lufs or the genre's — edm -8 club / -14 streaming — with the "
        + "shortfall and a maximizer/master-stage route), `tuning` (the kick's "
        + "fundamental from its DRY bus against the song's root, in cents, with the "
        + "beat rate and a daw_set_track route on hybrid_kick.tune), and dual-mono "
        + "`width` (L/R correlation >= 0.99 — a NOTE naming the rack's stereo switch, "
        + "master.stereo, when the render job had it off, since widening a side "
        + "signal that is exactly zero does nothing). Nothing is applied: this tool "
        + "only listens. "
        + HONESTY,
      inputSchema: {
        type: "object",
        ...
        properties: {
          ...
          key: {
            type: "string",
            description: "The song's root (F, Db, C#, 'F minor') for the TUNING critic, "
              + "which reads the kick's fundamental off its dry bus and reports the "
              + "cents to the root plus the beat rate against a sub on it. Omitted, "
              + "the root is read from the project (a key it carries, or a name like "
              + "'Big room F minor #1') or inferred from the bass line — and the "
              + "finding says which, at confidence 0.5 when inferred.",
          },
          target_lufs: {
            type: "number",
            description: "The LOUDNESS TARGET for this project (integrated LUFS). Beats "
              + "the genre's table: edm resolves to -8 (club) or -14 (streaming, with "
              + "`delivery`); other genres fall back to the -14 streaming default. The "
              + "loudness_target card quotes the source and the shortfall in dB.",
          },
          delivery: {
            type: "string",
            description: "club | streaming — which of the genre's loudness targets to "
              + "aim at when target_lufs is not given (edm defaults to club).",
          },
          profile: {
            type: "string",
            description:
              "§7 — the id of a REFERENCE PROFILE (daw_profile_list) to match this mix "
              + "against, adding up to six more findings: ref_bands, ref_level, "
              + "ref_kick_tune, ref_kick_decay, ref_pump and ref_width.\n"
              + "WHAT A PROFILE IS: dB numbers, times in milliseconds and counts, "
              + "measured off a track the owner has a copy of and separated into four "
              + "stems. It carries NO audio, no spectrogram frame, no note and no "
              + "melody — refprofile.py refuses to emit anything outside a declared "
              + "whitelist. Matching one makes a mix SIT like that record: the same "
              + "balance across nine bands, the same relative level for drums, bass, "
              + "other and vocals, a kick tuned and decaying like it, the same "
              + "sidechain depth and recovery, the same width per band. It cannot make "
              + "a mix SOUND like that record's parts, and nothing here reproduces any "
              + "of it.\n"
              + "AND IT IS NOT A TARGET. A reference is what somebody else did, not "
              + "what is correct, so every comparison is gain-invariant (a share "
              + "against a share, a decay as a ratio) and every ref_* finding is "
              + "ranked BELOW the objective critics — being 3 dB off pink is a "
              + "measurement, being 3 dB off a record is a preference. The reply's "
              + "`reference.skipped` lists every comparison that was NOT made and why; "
              + "read it, because a match that measured four of six things looks "
              + "exactly like one that found nothing wrong with the other two.",
          },
        },
      ...
      async run(a) {
        const r = await ear({
          ...
          key: a.key, target_lufs: a.target_lufs, delivery: a.delivery,
          profile: a.profile,
        });
        return {
          ...
          /* [DAWREC] What the critiqued mix CONTAINED: "notes+clips" when the
           * recorded takes were in the master and the stems, "notes" when
           * there were none, and the count of the clips that were in it. The
           * old `audio_clips_excluded` said 0 about a mix that had excluded
           * every one of them. */
          stems_cover: r.stems_cover, audio_clips: r.audio_clips,
          taste: r.taste,
