...
import {
  blankProject, blankTrack, blankAudioClip, blankClip, migrate,
  audioStartSample, audioEvents, audioJobClips, regionHashes, regionsOf, dirtyBetween,
...
console.log("\n  -- audio clips reach the region hashes (the dirty mechanism) --");
{
  ...
  doc.tracks[0].mute = false;

  /* ── SOLO, which `track.mute` alone never covered ────────────────────────
   * It did not have to while the chain path dropped clips on the floor: a
   * solo makes isDefaultMixer false, so a soloed document renders through the
   * rack, where a clip was silence whatever the audibility rule said. The
   * moment rack.py learned to mix clips, "mute only" would have meant a clip
   * on a NON-soloed track sounding straight through a solo — one silence
   * traded for a different lie. audioEvents reads mixerAudible now, which is
   * the test noteEvents has always used. */
  const solo = makeDoc();
  solo.tracks.push(blankTrack("other", "pluck"));
  solo.tracks[0].audioClips.push(blankAudioClip("imp_2.flac", {
    bar: 1, beat: 1, tick: 0, durSamples: SR,
  }));
  ok("with nothing soloed, the clip is audible", audioEvents(solo).length === 1);
  solo.tracks[1].solo = true;
  ok("SOLO on another track silences it, exactly as it silences that track's notes",
    audioEvents(solo).length === 0);
  solo.tracks[0].solo = true;
  ok("...and soloing its own track brings it back", audioEvents(solo).length === 1);
}

console.log("\n  -- a clip's region identity covers the chain its sound crosses --");
{
  /* THE HOLE THIS CLOSES. A region's hash covered the mixer strips of the
   * tracks with NOTES in it (`touched`), because notes were the only thing the
   * rack rendered. Now that a clip crosses its track's inserts, fader and pan,
   * a chain edit on a track carrying ONLY a recorded take changed the sound
   * and not the hash — and the stale render stayed on disk forever, which is
   * the one failure a content-addressed cache cannot recover from. */
  const doc = makeDoc();
  doc.master.stereo = true;                     // what store.js gives a NEW project
  doc.tracks[0].audioClips.push(blankAudioClip("imp_3.flac", {
    bar: 1, beat: 1, tick: 0, durSamples: SR * 2,
  }));
  const before = regionHashes(doc);
  /* A MEMORYLESS device first, because it is the half that must stay narrow:
   * `utility` is the catalog's stateful:false, so the clip keeps its P0 reach
   * and exactly one region moves. This is the assertion that would fail if the
   * reach rule below were applied to every insert instead of the stateful
   * ones — i.e. the one that keeps the sub-second edit loop. */
  doc.tracks[0].inserts.push({ id: "i0", type: "utility", enabled: true, params: {} });
  const dirtyMemoryless = dirtyBetween(before, regionHashes(doc), regionsOf(doc));
  ok("a MEMORYLESS insert on a clip-only track dirties the region the clip sounds "
    + "in, and only that one",
  dirtyMemoryless.length === 1 && dirtyMemoryless[0].fromBar === 1,
  JSON.stringify(dirtyMemoryless));
  doc.tracks[0].inserts = [];
  ok("...and removing it restores the hash exactly (the old render is valid again)",
    JSON.stringify(regionHashes(doc)) === JSON.stringify(before));

  /* ── AND THE STATEFUL HALF: A CLIP REACHES FORWARD LIKE A NOTE ──────────
   * The hole this closes came out of closing the one above. `eq` is stateful,
   * so once a clip crosses it the clip's sound is in every later region too —
   * clause 2 of mixer.js's DEPENDENCY-GRAPH RULE, which said NOTES because
   * notes were once the only thing the rack rendered. Measured on the tree
   * before store.audioEvents grew reach0/reach1: a one-second take through a
   * delay into a reverb put -26.2 dBFS of tail into the NEXT region, and that
   * region's hash did not move for ANY edit to the clip — a stale render
   * nothing could invalidate — while `bounce`, which concatenates the region
   * wavs, stepped 0.002510 at the seam against a correct 0.000039.
   * The control is right below it: the same edit on a track whose content is
   * a NOTE has always dirtied every region, and now they agree. */
  doc.tracks[0].inserts.push({
    id: "i1", type: "eq", enabled: true, params: { b2_gain_db: 6 },
  });
  const dirty = dirtyBetween(before, regionHashes(doc), regionsOf(doc));
  const allRegions = regionsOf(doc).length;
  ok("a STATEFUL insert on a clip-only track dirties the clip's region AND every "
    + "region its state reaches — the clip is not a shorter-lived note",
  dirty.length === allRegions && dirty[0].fromBar === 1,
  `${dirty.length} of ${allRegions}: ${JSON.stringify(dirty.map((d) => d.fromBar))}`);

  /* THE CONTROL. Same document, same insert, a NOTE instead of a clip: if
   * these two ever disagree again, one of them is wrong and this says which. */
  const noteDoc = makeDoc();
  noteDoc.master.stereo = true;
  noteDoc.tracks[0].clips.push({
    ...blankClip(1, 16, { name: "control" }),
    notes: [{ id: "n1", bar: 1, beat: 1, tick: 0, pitch: 60, durTicks: 480, vel: 100 }],
  });
  const noteBefore = regionHashes(noteDoc);
  noteDoc.tracks[0].inserts.push({
    id: "i1", type: "eq", enabled: true, params: { b2_gain_db: 6 },
  });
  const noteDirty = dirtyBetween(noteBefore, regionHashes(noteDoc), regionsOf(noteDoc));
  ok("...which is exactly what the same insert does to a track carrying a NOTE — "
    + "the two sources obey one rule",
  noteDirty.length === dirty.length,
  `note ${noteDirty.length} vs clip ${dirty.length}`);

  /* AND THE JOB, not only the hash. A tail that is hashed but not SENT is the
   * same silence one layer down: rack.chain_graph can only ring a device it
   * was given the clip for. audioJobClips is the one mapping every lane uses,
   * so this is the assertion that keeps the bounce's seam closed. */
  const regs2 = regionsOf(doc);
  const late = regs2[regs2.length - 1];
  const sent = audioJobClips(doc, "cap-test", late.t0, late.t1);
  ok("the clip is SENT to the last region's job too, so the graph can produce the "
    + "tail the hash now promises",
  sent.length === 1 && sent[0].track_id === doc.tracks[0].id,
  JSON.stringify(sent.map((s) => s.track_id)));
  doc.tracks[0].inserts = [];
  ok("...and with the stateful device gone the late region stops carrying it, so a "
    + "plain level mix is not paying for every clip in the song",
  audioJobClips(doc, "cap-test", late.t0, late.t1).length === 0);

  /* AND THE OTHER HALF: on a DEFAULT mixer nothing above may write a byte.
   * The clip loop moved above the MX block to feed `touched`, and a hash that
   * moved would strand every cached render on disk — so the sequence is
   * computed here from the documented format, by hand, rather than compared
   * against the code that produced it. */
  const p0 = makeDoc();
  p0.master.stereo = false;                     // the pre-switch document
  p0.tracks[0].audioClips.push(blankAudioClip("imp_4.flac", {
    bar: 1, beat: 1, tick: 0, durSamples: SR, gainDb: -6,
  }));
  const regs = regionsOf(p0);
  const a0 = audioEvents(p0)[0];
  const byHand = createHash("sha1")
    .update(`sr=${p0.sr};w=${regs[0].startSample}+${regs[0].nSamples};`)
    .update(`A|${a0.file}|${a0.offsetSamples}|${a0.durSamples}|${a0.startSample}|`
      + `${a0.gainDb.toFixed(3)};`)
    .digest("hex").slice(0, 12);
  ok("a DEFAULT mixer's hash is still notes-then-clips and nothing else — computed "
    + "by hand here, so a stray MX byte would fail rather than re-key every cache",
  regionHashes(p0)[0] === byHand, `${regionHashes(p0)[0]} vs ${byHand}`);
}
