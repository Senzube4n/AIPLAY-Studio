...
console.log("\n  -- the patch registry round-trips through a document --");
{
  ...
  /* ── THE BIG-ROOM SYNTHS (synths.py) sit in the same table by the same
   * rules, and hybrid_kick's `bigroom` PRESET is data the validator can
   * already take: every key a declared knob, every value in range. */
  const SYNTHS = ["bigroom_lead", "sub_bass", "riser", "impact"];
  ok("the four big-room synths are builtin patches -- no pack, no download",
    SYNTHS.every((p) => PATCHES[p] && PATCHES[p].kind === "builtin"
      && PATCHES[p].builtin === p && !PATCHES[p].pack && PATCH_IDS.includes(p)));
  ok("...each with a front panel of >= 6 min/max/default/unit/doc knobs",
    SYNTHS.every((p) => {
      const s = PATCHES[p].params;
      return s && Object.keys(s).length >= 6 && Object.values(s).every((k) =>
        Number.isFinite(k.min) && Number.isFinite(k.max) && Number.isFinite(k.default)
  ...
  ok("a lead knob survives normParams clamped, and is dropped on a drum machine",
    normParams({ filter_decay: 45.5, resonance: 9 }, "bigroom_lead").filter_decay === 45.5
    && normParams({ resonance: 9 }, "bigroom_lead").resonance === 1
    && normParams({ filter_decay: 45.5 }, "tr808").filter_decay === undefined);
  const preset = PATCHES.hybrid_kick.presets?.bigroom;
  ok("hybrid_kick carries a `bigroom` preset: a doc that states what was measured, and params",
    !!preset && /T60/.test(preset.doc) && typeof preset.params === "object");
  ok("...every preset key is a declared hybrid_kick knob inside its range, so normParams keeps "
    + "what differs from the default and drops nothing the preset means",
    !!preset && Object.entries(preset.params).every(([k, v]) => {
      const s = PATCHES.hybrid_kick.params[k];
      return s && v >= s.min && v <= s.max;
    })
    && Object.entries(normParams(preset.params, "hybrid_kick")).every(([k, v]) => preset.params[k] === v)
    && Object.keys(normParams(preset.params, "hybrid_kick")).length
       === Object.entries(preset.params).filter(([k, v]) => v !== PATCHES.hybrid_kick.params[k].default).length);
}
