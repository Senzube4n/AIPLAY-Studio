...
WHAT IS MODELLED (and what is honestly approximated)
  ...
  tr909   Deliberately a DIFFERENT circuit, not the 808 with other numbers.
          ...
          LAYERS (2026-09-03), all OFF by default so every existing render
          keeps its bytes: the clap gains a full-band white-noise burst with
          its own decay (`clap_noise`, `clap_noise_decay`) and a darker
          in-voice room tail (`clap_room`); the hats gain a velocity law
          that shortens the decay as well as the level (`hat_vel`) and a
          stereo width that turns the right channel's phase through a
          first-order allpass (`hat_width`) on top of the kit's pan. The
          owner heard the clap and hat as "a bit simple": they were one
          band-passed noise and one placed-mono cluster each.
...
def _ceiling(y, peak=0.95):
    """A safety ceiling, NOT a normaliser: touches nothing under `peak`.
    The layered voices (a clap with its burst and room, a sub with its mid
    layer, a lead with its octave stack) can sum past full scale at a knob's
    extreme at velocity 127; the seam's promise is that no voice clips
    there. At defaults every caller skips the sum and this never runs."""
    m = float(np.max(np.abs(y)))
...
# the hats' width: ONE first-order allpass on the right channel, its corner
# placed so the phase at 10 kHz (where a closed hat lives) is turned by
# 90 degrees x width. Magnitude flat at every width -- the right hat is as
# loud as the left whatever the knob (a crossfade with a shifted copy was
# tried first: it cost the right channel 6.7 dB at 0.6 and left the pair
# ANTI-correlated at 1, -0.63, which the fold paid for at -9.8 dB). A 90
# degree turn is a correlation of about 0 -- two waveforms of one spectrum,
# not one waveform and its negative.
_WIDTH_HZ = 10000.0
_HAT_KEYS = (42, 44, 46)


def _widen(x, w, sr):
    """The right channel through one allpass whose corner sits where the
    phase at 10 kHz is 90 degrees x w. The caller skips this at w 0 (the
    corner would be at Nyquist, the identity); at 1 the corner is 10 kHz.
    The FOLD's (L+R)/2 loses up to 3 dB in the band at 1 -- the price of
    any decorrelation under a mono bus -- and instruments_test measures
    it."""
    k_ref = math.tan(math.pi * _WIDTH_HZ / sr)
    k = k_ref / math.tan(0.25 * math.pi * min(max(float(w), 1e-4), 1.0))
    a = (k - 1.0) / (k + 1.0)
    return sosfilt(np.array([[a, 1.0, 0.0, 1.0, a, 0.0]]), x)
...
def _hat_vel(P, v, tau, level):
    """The hats' velocity law beyond the level trim every voice has: at
    hat_vel 1 a soft hit is SHORTER as well as quieter (tau x 0.4 and level
    x 0.5 at velocity 1, unchanged at 127). hat_vel 0 returns the arguments
    untouched -- not multiplied by 1.0, untouched -- so the default is the
    old hat to the byte."""
    hv = float(P["hat_vel"])
    if hv > 0.0:
        tau = tau * (1.0 - 0.6 * hv * (1.0 - v))
        level = level * (1.0 - 0.5 * hv * (1.0 - v))
    return tau, level


def _909_ch(P, sr, v):
    tau, level = _hat_vel(P, v, 0.007 + 0.026 * float(P["hat_decay"]), 3.6 * (0.55 + 0.45 * v))
    return _909_metal(sr, tau, 8000.0, (8000.0, 15000.0), level)


def _909_oh(P, sr, v):
    tau, level = _hat_vel(P, v, 0.035 + 0.200 * float(P["openhat_decay"]), 3.1 * (0.6 + 0.4 * v))
    return _909_metal(sr, tau, 6500.0, (6500.0, 15000.0), level)
...
def _909_cp(P, sr, v):
    """The clap, plus two layers that are OFF at 0 (the code path is skipped,
    so the old clap is the old bytes): a white-noise BURST -- high-passed at
    200 Hz only, so it reaches the air the 1.2-5.2 kHz band-pass keeps the
    clap out of -- with its own decay; and a ROOM, a darker (400 Hz - 3.5
    kHz) noise tail that rises over 25 ms from 15 ms in and falls with a
    200 ms time constant -- twice the clap's own tail, so it OUTLASTS it
    (the voice grows from 0.60 s to 1.10 s to hold it, cosine-faded). Both
    are their own noise seeds, so they are layers, not the same noise
    louder. The sum sits under a 0.95 ceiling that never runs at 0."""
    room = float(P["clap_room"])
    n = int(sr * (1.10 if room > 0.0 else 0.60))
    ...
    y = y * env * 1.30
    nz = float(P["clap_noise"])
    if nz > 0.0 or room > 0.0:
        t = _t(n, sr)
        if nz > 0.0:
            burst = _norm(_filt(_noise(n, sr, 4244), "hp", 200.0, sr, 2)
                          * np.exp(-t / (float(P["clap_noise_decay"]) / 1000.0)))
            y = y + burst * (nz * 0.36)
        if room > 0.0:
            on = int(0.015 * sr)
            ramp = int(0.025 * sr)
            renv = np.zeros(n)
            renv[on:] = np.exp(-t[:n - on] / 0.200)
            renv[on:on + ramp] *= np.linspace(0.0, 1.0, ramp)
            tail = _norm(_filt(_noise(n, sr, 4245), "bp", (400.0, 3500.0), sr, 2) * renv)
            y = _fade_tail(y + tail * (room * 0.85))
        y = _ceiling(y)
    return y
...
def voice(name, midi, dur_samples, vel127, sr, params=None):
    ...
    L, R = mono * gl, mono * gr
    # the hats' width: the RIGHT channel decorrelated, the left the old hat,
    # on top of the pan above. Only where the knob exists and is up.
    w = float(P.get("hat_width", 0.0))
    if w > 0.0 and spec["kit"] is not None and int(midi) in _HAT_KEYS:
        R = _widen(R, w, sr)
