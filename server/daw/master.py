...
one JSON object:
  ...
  check_delivery   PASS/FAIL against the streaming targets + the exact gain
                   (+ `stage`: what the bounce's loudness stage would do, and
                   a `custom` row when the job carries target_lufs)

Plus ONE processing stage that is not a device: loudness_stage(), the second
pass of a two-pass bounce (instruments.py::_cli_encode calls it when the
encode job carries target_lufs). See THE LOUDNESS STAGE below for why it is
a pass over the assembled master and not an insert in the master chain.
...
def check_delivery(job, synths):
    ...
    for t in DELIVERY_TARGETS:
        ...
        # What the bounce's loudness stage would do for THIS target: the same
        # arithmetic it runs (plan_loudness), so the advice and the bounce
        # cannot disagree about whether a target is reachable.
        row["stage"] = plan_loudness(li, tp, float(t["lufs"]), float(t["true_peak_db"]))
        ...
        rows.append(row)
    # A caller's own target (the arranger's -8 for a club master) gets the
    # same treatment as a published one, under id "custom", so the Ear can
    # ask "will the bounce reach it?" without bouncing.
    custom = job.get("target_lufs")
    if custom is not None and custom != "" and (not only or "custom" in only):
        rows.append(custom_delivery_row(li, tp, float(custom),
                                        float(job.get("ceiling_db", LOUDNESS_CEILING_DB)),
                                        float(job.get("max_limit_db", LOUDNESS_MAX_LIMIT_DB))))
    return {
        "ok": True, "sr": sr, **info,
...
# ═══════════════════════════════════════════════════════════════════════════
# THE LOUDNESS STAGE -- the second pass of a two-pass bounce
# ═══════════════════════════════════════════════════════════════════════════
#
# WHY A PASS AND NOT A DEVICE. A region render is one window of the chain
# graph processed from absolute sample 0, which is what makes region seams
# exact by construction. Integrated loudness is a property of the WHOLE song
# (gated over every 400 ms block of it), so a gain that aims at a target
# cannot be computed inside one region without every other region -- a
# device in the master chain would land a different gain in each region and
# put a step at every seam. The one place the whole bounce is one buffer is
# instruments.py::_cli_encode, where the regions are concatenated: that is
# where this runs, before the dither and the write, on the BS.1770 meter
# (rack.lufs_integrated) and the true-peak limiter (rack.limiter_gain) the
# rack already trusts. Nothing here is a second implementation of either.
#
# WHY AFTER THE TANH. rack.chain_graph ends in tanh(0.7 x), memoryless, per
# sample -- the P0 seam proof's master curve, which stays last in the rack.
# Its ceiling is asymptotic: a -1 dBTP master limiter feeding it lands at
# tanh(0.7 * 0.891) = -5.1 dBFS, which is why the provers' bounce sat at
# -7.2 dBTP with the master limiter never engaging. Gain applied BEFORE the
# tanh would drive the curve into audible saturation to buy loudness; gain
# applied here, on the assembled file, is clean and the limiter is the only
# thing that touches the peaks.
#
# WHAT IT DOES, in order:
#   1. measure  integrated LUFS and true peak of the assembled master.
#   2. plan     the gain that lands on the target, CAPPED so the limiter
#               after it never has more than `max_limit_db` (3 dB) to remove.
#               The cap is the honesty: a limiter doing 9 dB is a different
#               record (pumping, flattened kicks), and the caller asked for a
#               loudness, not for that. When the cap binds the target is NOT
#               reached, and the report says by how much and what it would
#               take.
#   3. apply    gain, then the limiter at `ceiling_db` (-1 dBTP: the ceiling
#               every DELIVERY_TARGET shares).
#   4. re-measure. Limiting shaves loudness off the loudest blocks (a
#               kick's transient is most of a block's energy), so a first
#               pass lands under -- measured: 2 dB of limiting cost ~0.9 LU
#               on the synthetic drop. While the cap has room the gain is
#               topped up by the miss and 3-4 run again FROM THE ORIGINAL
#               buffer (the limiter runs once, on the final gain, never on
#               its own output), up to `rounds` times; the report names the
#               gain that was actually rendered, never a top-up that was not.
#
# WHAT IT REFUSES: to raise the ceiling, or to spend more than the budget.
# A mix that cannot reach its target inside 3 dB of limiting has too high a
# crest for that target -- true peak over LUFS > ceiling + budget - target
# (10 dB for -8 LUFS at -1 dBTP) -- and the fix is upstream: lead level,
# sidechain release, density. The report names the crest and the allowance
# so the Ear can route the finding to a mix knob instead of the master.

LOUDNESS_CEILING_DB = -1.0
LOUDNESS_MAX_LIMIT_DB = 3.0
LOUDNESS_TOLERANCE_LU = 0.5


def plan_loudness(lufs, true_peak_db, target_lufs, ceiling_db=LOUDNESS_CEILING_DB,
                  max_limit_db=LOUDNESS_MAX_LIMIT_DB):
    """The arithmetic on its own, so check_delivery's advice and the stage's
    behaviour are one calculation. All dB; `lufs` finite."""
    lufs, tp, target = float(lufs), float(true_peak_db), float(target_lufs)
    ceiling, budget = float(ceiling_db), float(max_limit_db)
    want = target - lufs                       # the gain that lands on target
    allowed = ceiling + budget - tp            # the most gain the budget permits
    gain = min(want, allowed)
    return {
        "gain_db": round(gain, 3),
        "limiting_db": round(max(tp + gain - ceiling, 0.0), 3),
        "expected_lufs": round(lufs + gain, 2),
        "shortfall_db": round(max(want - gain, 0.0), 2),
        "reachable": bool(want <= allowed + 1e-9),
        # the two numbers that decide reachability, for the advice
        "crest_tp_db": round(tp - lufs, 2),
        "crest_allowed_db": round(ceiling + budget - target, 2),
    }


def loudness_stage(y, sr, target_lufs, ceiling_db=LOUDNESS_CEILING_DB,
                   max_limit_db=LOUDNESS_MAX_LIMIT_DB, release_ms=80.0,
                   lookahead_ms=5.0, rounds=4):
    """(2, N) float64 in -> (2, N) float64 out, plus the report. Pure: same
    buffer, same bytes. Mono callers pass dual mono and take row 0."""
    R = _rack()
    y = np.asarray(y, dtype=np.float64)
    if y.ndim == 1:
        y = np.vstack([y, y])
    target, ceiling, budget = float(target_lufs), float(ceiling_db), float(max_limit_db)
    li = R.lufs_integrated(y, sr)
    tp = R.true_peak_db(y)
    base = {
        "target_lufs": target, "ceiling_db": ceiling, "max_limit_db": budget,
        "before": {"lufs": None if not math.isfinite(li) else round(li, 2),
                   "true_peak_db": round(tp, 2)},
    }
    if not math.isfinite(li):
        return y, dict(base, applied=False, reached=False, gain_db=0.0,
                       limiter_work_db=0.0, after=dict(base["before"]), rounds=0,
                       shortfall_db=None,
                       advice="This bounce measures as silence -- nothing to aim.")
    plan = plan_loudness(li, tp, target, ceiling, budget)
    allowed = ceiling + budget - tp
    # Every gain that is rendered is a 0.001 dB step: the report's gain_db IS
    # the rendered gain, and gain-then-limiter from the report reproduces the
    # file (master_test.py checks exactly that).
    gain = round(min(target - li, allowed), 3)
    lp = R.Params(R.CATALOG["limiter"],
                  {"ceiling_db": ceiling, "release_ms": float(release_ms),
                   "lookahead_ms": float(lookahead_ms)}, sr)
    ctx = {"sr": sr, "spq": [[0.0, 0.5]], "dry": {}}
    z, work, after, used = y, 0.0, li, 0
    rounds = max(1, int(rounds))
    while True:
        used += 1
        z = y * R.db_to_lin(gain)
        work = 0.0
        if tp + gain > ceiling - 1e-9:
            g = R.limiter_gain(z, lp, ctx)
            work = -20.0 * math.log10(max(float(np.min(g)), 1e-12))
            z = z * g
        after = R.lufs_integrated(z, sr)
        miss = target - after
        room = allowed - gain
        # Stop when on target, when the budget is spent, or when this was
        # the last round -- the gain reported below is the gain rendered.
        if miss <= 0.1 or room < 1e-3 or used >= rounds:   # 1e-3: one gain quantum
            break
        gain = round(gain + min(miss, room), 3)   # top up, never past the budget
    reached = bool(math.isfinite(after) and abs(after - target) <= LOUDNESS_TOLERANCE_LU)
    shortfall = max(target - after, 0.0) if math.isfinite(after) else None
    tp_after = R.true_peak_db(z)
    if reached:
        advice = (f"On target: {after:.2f} LUFS with {work:.2f} dB of limiting "
                  f"at {ceiling:g} dBTP (gain {gain:+.2f} dB).")
    else:
        advice = (f"{shortfall:.2f} LU short of {target:g} LUFS: the limiter budget "
                  f"({budget:g} dB) is spent at {gain:+.2f} dB of gain. Reaching it from "
                  f"here would take about {work + shortfall:.1f} dB of limiting. The mix's "
                  f"crest is {plan['crest_tp_db']:.1f} dB (true peak over LUFS) against the "
                  f"{plan['crest_allowed_db']:.1f} dB the target allows -- close that gap "
                  "upstream (lead level, sidechain release, density), not at the master.")
    return z, dict(base, applied=True, gain_db=round(gain, 3), limiter_work_db=round(work, 3),
                   after={"lufs": round(after, 2), "true_peak_db": round(tp_after, 2)},
                   reached=reached, rounds=used,
                   shortfall_db=None if shortfall is None else round(shortfall, 2),
                   plan=plan, advice=advice)


def custom_delivery_row(li, tp, target, ceiling_db=LOUDNESS_CEILING_DB,
                        max_limit_db=LOUDNESS_MAX_LIMIT_DB):
    """check_delivery's row for a caller-named target -- the arranger's club
    master -- judged the way the stage will judge it."""
    row = {
        "id": "custom", "label": f"custom {target:g} LUFS", "lufs": float(target),
        "tolerance_lu": LOUDNESS_TOLERANCE_LU, "true_peak_db": float(ceiling_db),
        "confidence": "caller",
        "source": "the job's target_lufs (what the bounce's loudness stage aims at)",
        "note": "Not a platform. The stage reaches it when the mix's crest "
                "(true peak over LUFS) is inside ceiling + limiter budget - target.",
        "measured_lufs": li, "measured_true_peak_db": tp,
    }
    if li is None:
        row.update(lufs_pass=None, true_peak_pass=tp <= ceiling_db + 1e-9, stage=None,
                   gain_change_db=None, verdict="silent",
                   advice="This window measures as silence -- nothing to aim.")
        return row
    plan = plan_loudness(li, tp, target, ceiling_db, max_limit_db)
    delta = float(target) - float(li)
    row.update(lufs_delta_lu=round(delta, 2), gain_change_db=round(delta, 2),
               lufs_pass=bool(abs(delta) <= LOUDNESS_TOLERANCE_LU + 1e-9),
               true_peak_pass=bool(tp <= ceiling_db + 1e-9), stage=plan,
               limiting_needed_db=plan["limiting_db"])
    row["pass"] = bool(row["lufs_pass"] and row["true_peak_pass"])
    if row["pass"]:
        row["verdict"], row["advice"] = "pass", "Within tolerance and under the ceiling."
    elif plan["reachable"]:
        row["verdict"] = "reachable"
        row["advice"] = (f"The stage will apply {plan['gain_db']:+.2f} dB and the limiter "
                         f"about {plan['limiting_db']:.2f} dB to land at {target:g} LUFS.")
    else:
        row["verdict"] = "needs a denser mix"
        row["advice"] = (f"{plan['shortfall_db']:.2f} LU short even with {max_limit_db:g} dB "
                         f"of limiting: crest {plan['crest_tp_db']:.1f} dB against the "
                         f"{plan['crest_allowed_db']:.1f} dB the target allows. Raise the "
                         "mix's density (lead level, sidechain release) before the master.")
    return row


# ═══════════════════════════════════════════════════════════════════════════
# BOUNCE WIRING -- where dither and the loudness stage are called
# ═══════════════════════════════════════════════════════════════════════════
#
# apply_dither() and loudness_stage() above are export-time functions. The
# bit-depth reduction in this repo happens in ONE place:
#
#     server/daw/instruments.py :: _cli_encode()   ->  sf.write(..., subtype)
#
# and that function takes `bit_depth` (16 or 24) and `dither`, calling
# master.apply_dither() before the write whenever the depth is under 24, and
# now `target_lufs` (+ optional `ceiling_db`, `max_limit_db`), calling
# master.loudness_stage() BEFORE the dither. The route that builds that job is
#
#     server/daw/routes.js :: case "bounce"  ->  runInstruments("encode", {...})
#
# which is owned by another agent, so it is NOT edited here. bit_depth is
# already passed through. For the loudness stage ONE LINE is needed there,
# reading the project's setting (the arranger sets it for a club master):
#
#     const enc = await runInstruments("encode", {
#       sr: doc.sr, wav_parts: parts, out,
#       ...(b.bit_depth !== undefined ? { bit_depth: b.bit_depth } : {}),
#       ...(doc.master?.target_lufs != null ? { target_lufs: doc.master.target_lufs } : {}),
#     }, 300_000);
#
# and the setting itself lives on doc.master (`target_lufs`, number or
# absent) beside `stereo`, set through mixer_set / daw_set_master like any
# other master value -- one door. Absent means off: the bytes the bounce
# wrote before this stage existed. The encode reply then carries `loudness`
# (before/after, gain, limiter work, reached, shortfall, advice) for the
# route to return and the Ear to read.
