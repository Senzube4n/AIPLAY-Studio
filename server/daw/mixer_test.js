...
import {
  ...
  isDefaultMixer, mixerAudible, mixerReach, trackStatefulPath, handleMixerAction,
...
function makeDoc() {
  ...
  /* An EXISTING document: written before the stereo switch, so it has no
   * `stereo` key and renders through the fold. blankProject turns the switch
   * on for NEW projects — the block at the end covers that. */
  delete doc.master.stereo;
...
console.log("\n  -- THE STEREO SWITCH and the LOUDNESS TARGET on the master strip --");
{
  const fresh = blankProject("new one", { bpm: 128, num: 4, den: 4, lengthBars: 16 });
  ok("a NEW project starts with master.stereo true (the voices' own channels)",
    fresh.master.stereo === true && fresh.master.target_lufs === undefined);
  const doc = makeDoc();                       // an existing document: no key
  note(doc, "trk_A", 1);
  note(doc, "trk_B", 14);
  ok("an existing document has no stereo key after migrate, and is default (the fold, P0 bytes)",
    doc.master.stereo === undefined && isDefaultMixer(doc) && mixerSigBundle(doc) === null);
  const h0 = regionHashes(doc);
  const dOn = dirtyOf(doc, (x) => { x.master.stereo = true; });
  ok("turning the switch on makes the mixer non-default (rack path) and dirties EVERY sounding region",
    !isDefaultMixer(doc) && dOn.length === 2 && dOn[0].idx === 0 && dOn[1].idx === 3, JSON.stringify(dOn));
  ok("the master signature carries the switch", /"s":1/.test(mixerSigBundle(doc).master));
  ok("the render job carries mixer.stereo: true — the key rack.stereo_on reads",
    mixerJobPayload(doc).stereo === true);
  doc.master.stereo = false;
  ok("only a literal true is the switch: false renders the fold, the default mixer and the P0 hashes exactly",
    isDefaultMixer(doc) && mixerJobPayload(doc).stereo === false
    && JSON.stringify(regionHashes(doc)) === JSON.stringify(h0));
  const m1 = migrate(JSON.parse(JSON.stringify({ ...doc, master: { inserts: [], fader: 0, stereo: "yes" } })));
  const m2 = migrate(JSON.parse(JSON.stringify({ ...doc, master: { inserts: [], fader: 0, stereo: true, target_lufs: -8 } })));
  const m3 = migrate(JSON.parse(JSON.stringify({ ...doc, master: { inserts: [], fader: 0, target_lufs: "-99" } })));
  ok("migrate keeps stereo only as a literal true, and target_lufs clamped into -30..-6",
    m1.master.stereo === undefined && m2.master.stereo === true && m2.master.target_lufs === -8
    && m3.master.target_lufs === -30 && m3.master.stereo === undefined);
  doc.master.stereo = true;
  const hOn = regionHashes(doc);
  const dT = dirtyOf(doc, (x) => { x.master.target_lufs = -8; });
  ok("the loudness target is a BOUNCE-time setting: it dirties no region and enters no hash",
    dT.length === 0 && JSON.stringify(regionHashes(doc)) === JSON.stringify(hOn)
    && mixerJobPayload(doc).master.target_lufs === undefined);
}

/* mixer_set on the master: through handleMixerAction with a mutate stub */
{
  const doc = makeDoc();
  const mutate = async (slug, b, action, fn) => ({ doc, dirty: [], extra: fn(doc) || {} });
  const ctx = { mutate, readProject: async () => doc, runEngineFast: async () => ({}), safe: (s) => String(s) };
  const r1 = await handleMixerAction("mixer_set", { slug: "x", target: "master", stereo: true, target_lufs: -8 }, ctx);
  ok("mixer_set target=master stereo=true target_lufs=-8 writes both and answers with both",
    doc.master.stereo === true && doc.master.target_lufs === -8 && r1.stereo === true && r1.target_lufs === -8);
  const r2 = await handleMixerAction("mixer_set", { slug: "x", target: "master", stereo: false, target_lufs: null }, ctx);
  ok("...and false / null remove the keys (the document reads like one written before the switch)",
    doc.master.stereo === undefined && doc.master.target_lufs === undefined && r2.stereo === false && r2.target_lufs === null);
  let refused = null;
  try { await handleMixerAction("mixer_set", { slug: "x", target: "trk_A", stereo: true }, ctx); } catch (e) { refused = e.message; }
  ok("a track has no stereo switch (it is the rack's, on the master)", /master/.test(refused || ""), refused);
  refused = null;
  try { await handleMixerAction("mixer_set", { slug: "x", target: "master", target_lufs: "loud" }, ctx); } catch (e) { refused = e.message; }
  ok("a target that is not a number is refused, not stored", /number/.test(refused || "") && doc.master.target_lufs === undefined);
  const r3 = await handleMixerAction("mixer_set", { slug: "x", target: "master", target_lufs: -5 }, ctx);
  ok("the target clamps to the loud end (-6)", r3.target_lufs === -6);
}
