...
── STEREO THROUGH THE RACK (mixer.stereo) ──────────────────────────────────
Every voice the instrument stage returns is (2, N) float64 -- a unison lead
with its voices panned, a sampled piano with its real image -- and until this
switch existed the rack folded each one to (L+R)/2 in _synth_notes and then
duplicated the fold into both channels before the first insert. A 0.70 L/R
correlation at the voice was 1.000 at the bounce: the file was stereo in
format and mono to the byte. The provers found it by comparing the two
channels of a 4:00 bounce; nothing sounded "wrong".

  mixer.stereo: true    dry buffers are (2, N): the voice's own channels,
                        accumulated per track, straight into the inserts.
  mixer.stereo: false   the fold, exactly as before. THIS IS THE DEFAULT
  (or absent)           inside this file, because a project rendered under
                        the fold has cached regions and pinned bounces that
                        must stay byte-identical (rack_test.py pins the
                        sha1 of two such renders). store.js turns it ON for
                        NEW projects only; an existing document opts in by
                        setting master.stereo, which dirties every region
                        the way any master change does.

What the switch does NOT change: the sidechain. A compressor keyed from
another track reads that track's DRY signal, and a key is mono by nature --
under the switch it is (L+R)/2 of the SUMMED stereo track (the same value
the fold produced when the voice was mono, exactly: x+x is exact in IEEE
and so is the halving). Every dynamics device in this file already computes
ONE gain from max(|L|, |R|) and applies it to both channels, so a stereo
track through a compressor, gate, limiter or maximizer is never pumped
differently left and right; the M/S wrappers in master.py were written for
(2, N) and meet the stereo buffers unchanged. Pan law, sends, returns, the
master chain and the tanh are per channel already.
"""
import hashlib
import json
import math
...
def limiter_gain(x, p, ctx):
    """The limiter's per-sample gain curve (n,), 0 < g <= 1, for (2, N) in.
    Factored out of dev_limiter so master.loudness_stage can report how much
    work the ceiling did (min gain in dB) without a second implementation of
    the detector; dev_limiter is exactly `x * limiter_gain(x, p, ctx)`."""
    sr = ctx["sr"]
    n = x.shape[1]
    ...
    # _ctrl_pool zero-pads partial blocks, which is correct for energy/max
    # but would invent zero gain (infinite reduction) at a non-grid ending.
    # Unity is the neutral padding value for a minimum of gains in [0,1].
    g_c = _ctrl_pool(np.pad(g_inst, (0, (-n) % CTRL), constant_values=1.0), np.min)
    m = len(g_c)
    rel_c = _coef(p.env("release_ms", n, CTRL, sr)[:m], sr)
    out = np.empty(m)
    e = 1.0
    ...
    win /= win.sum()
    return np.convolve(np.pad(g_min, la, mode="edge"), win, mode="valid")[:n]


def dev_limiter(x, p, ctx):
    return x * limiter_gain(x, p, ctx)
...
def stereo_on(mixer):
    """The switch, read in ONE place: mixer.stereo is true, and only true."""
    return (mixer or {}).get("stereo") is True


def _synth_notes(job, synths, sr, total, stereo=False):
    """The dry buffers: notes -> one float64 buffer per track, P0 note maths
    to the letter (clamps, seed, the 0.5 headroom, absolute placement).
    Mono (N,) under the fold, (2, N) under mixer.stereo -- see the header."""
    dry = {}
    ...
    for note in job.get("notes") or []:
        ...
        if tid not in dry:
            dry[tid] = np.zeros((2, total)) if stereo else np.zeros(total)
        midi = int(note["midi"])
...
def _mix_audio(job, sr, total, stereo, dry):
    """[DAWREC] The dry buffers' OTHER source: file-backed clips, into the
    owning track's buffer, with the P0 path's semantics to the sample.

    NOT a second implementation: engine.mix_clips IS the placement, called
    here on each track's dry buffer with w0=0 because this stage processes
    from absolute sample 0. So the trim, the gain, the intersection and the
    mono-into-both-channels rule are the same code the mono lane runs, and
    capture_test.py proves it by bit-comparing one lane against the other.

    A clip's track is `track_id`, read by exactly the rule _synth_notes reads
    a note's: absent means the unnamed bus (""), which a mixer has no config
    for and therefore passes at unity — audible, which is the honest answer
    for a job that named no track, and never silence.

    The import is lazy for the reason master.py's is: engine.py imports this
    module at its own bottom, so the cycle stays open only if the call back
    happens at call time.
    """
    clips = job.get("audio") or []
    if not clips:
        return dry                       # a job with no clips touches nothing
    import engine                        # noqa: PLC0415 -- see the docstring
    per = {}
    for clip in clips:
        per.setdefault(str(clip.get("track_id") or ""), []).append(clip)
    for tid, rows in per.items():
        if tid not in dry:
            dry[tid] = np.zeros((2, total)) if stereo else np.zeros(total)
        engine.mix_clips(rows, dry[tid], 0, sr)
    return dry
...
def graph_future_seconds(mixer):
    ...
    future notes/clips in region jobs AND their cache hashes.
    """
    def chain(rows):
        seconds = 0.0
        for ins in rows or []:
            ...
            if isinstance(value, dict):
                values = [float(k.get("v", 5)) for k in value.get("keys", [])]
                value = max(values or [5])
            seconds += 2 * min(10.0, max(1.0, float(value))) / 1000 + .002
        return seconds
    returns = {str(r.get("id")): chain(r.get("inserts")) for r in mixer.get("returns", [])}
    paths = [0.0]
    for track in (mixer.get("tracks") or {}).values():
        tail = max([0.0] + [returns.get(str(s.get("to")), 0.0) for s in track.get("sends", [])])
        paths.append(chain(track.get("inserts")) + tail)
    return max(paths) + chain((mixer.get("master") or {}).get("inserts"))
...
def chain_graph(job, synths, capture=False):
    ...
    # [DAWREC] ...AND THE FILE-BACKED CLIPS, into the same buffers, BEFORE
    # anything reads them. Not one line of this module knew the word "audio"
    # until now, so a recorded or imported take rendered as SILENCE the moment
    # a project had a mixer -- which store.js gives every NEW project
    # (master.stereo, and stereo_on forces this path). Measured: the same
    # one-second clip, peak 0.206966 through engine.render's mono path and
    # peak 0.0 here, with the sha1 of the same job carrying no clip at all.
    # It must sit HERE, above `keys`, so a compressor keyed off a recorded
    # track hears the recording rather than an empty bus.
    _mix_audio(job, sr, total, stereo, dry)
    # The sidechain key is mono by nature: under the switch it is (L+R)/2 of
    # the summed stereo track; under the fold it is the fold itself.
    keys = {tid: (d[0] + d[1]) * 0.5 for tid, d in dry.items()} if stereo else dry
    ctx = {"sr": sr, "spq": mixer.get("spq") or [[0.0, 0.5]], "dry": keys,
           "stereo": stereo}
    ...
    for tid in sorted(dry):
        cfg = tcfg.get(tid) or {}
        x = dry[tid] if stereo else np.vstack([dry[tid], dry[tid]])
    ...
    for r in returns:
        ...
        if capture:
            buses["returns"][rid] = y
    if capture:
        # [VOICELAB] THE PRE-MASTER SUM, captured before the master chain
        # touches it. This is the buffer the per-track lanes really add up to:
        # `mastered` is master chain -> fader -> tanh ON TOP of it, so a lane
        # view that claims to sum to the master is wrong at the limiter, and
        # the panel has to say so. Purely additive -- `mastered` is computed
        # from the same `mix` either way, and nothing downstream iterates this
        # dict's keys (ear.py:1121 and meters() index "tracks"/"returns" by
        # name), so not one existing byte moves.
        buses["mix"] = mix
    ...
    if future:
        mastered = mastered[:, :requested_total]
        if capture:
            buses["tracks"] = {k: y[:, :requested_total] for k, y in buses["tracks"].items()}
            buses["returns"] = {k: y[:, :requested_total] for k, y in buses["returns"].items()}
            buses["mix"] = buses["mix"][:, :requested_total]
...
def render_with_chain(job, synths, tails):
    ...
    return {"ok": True, "sr": sr, "n_samples": n, "channels": 2,
            ...
            "notes": len(job.get("notes") or []),
            # [DAWREC] said in the same breath as the note count, because for
            # as long as this reply named only notes, a job whose whole
            # content was a clip reported "ok" over a silent file.
            "clips": len(job.get("audio") or []), "chained": True,
            "stereo": stereo_on(job.get("mixer")),
            "ms": round((time.perf_counter() - t_start) * 1000, 1)}


# ------------------------------------------------- [VOICELAB] render_stems
#
# A SEPARATE MODE, and separate on purpose. Every region hash on disk and the
# region-seam proof stand on `render`/`render_with_chain` producing the bytes
# they produced yesterday, so nothing here is reachable from either: the only
# line above that this feature touched is the `buses["mix"]` capture inside
# chain_graph, which runs solely when a caller already asked for capture.
#
# WHAT A STEM IS, exactly. buses["tracks"][tid] is the POST-FADER, post-pan,
# post-insert stereo bus of one track, out of THE SAME graph the mix came
# from -- nothing is re-synthesised and no second signal path exists. Two
# facts follow, and both are reported in the payload rather than left for the
# UI to guess:
#   1. the stems sum to the PRE-MASTER mix, not to the master. `residual_db`
#      is measured, not asserted -- it is -inf (exactly zero error) when the
#      sum is bit-identical, which is what the arithmetic says it must be.
#   2. `master_delta_db` says how far the mastered output sits from that sum.
#      It is the differential guard: if it ever reads -inf, the master chain
#      is not in the signal path and every lane is quietly lying about it.


def _sum_buses(buses, total):
    """Re-add the captured buses in chain_graph's OWN accumulation order --
    tracks in sorted(track_id) order, then returns in job order. Float
    addition is not associative, so the order is the claim."""
    s = np.zeros((2, total))
    for tid in sorted(buses["tracks"]):
        s += buses["tracks"][tid]
    for rid in buses["returns"]:                 # dicts keep insertion order
        s += buses["returns"][rid]
    return s


def _err_to_sig_db(err, sig):
    """Error-to-signal in dB. -inf means bit-identical, and it is allowed to."""
    e = float(np.sum(np.square(err)))
    p = float(np.sum(np.square(sig)))
    if e <= 0.0:
        return float("-inf")
    if p <= 0.0:
        return float("inf")
    return 10.0 * math.log10(e / p)


def render_stems(job, synths, tails=None):
    """engine.py MODE `render_stems`: one absolute sample window through the
    SAME graph, written one float32 stereo wav per track and shared return.

    job: the `render` job that carries `mixer`, plus
         out_dir   where the files land (required)
         prefix    filename prefix, so the caller can key them by the region
                   hash: "reg7_1a2b3c4d5e6f_" -> reg7_..._trk_<tid>.wav
    """
    t_start = time.perf_counter()
    sr = int(job.get("sr") or 48000)
    w0 = int(job["start_sample"])
    n = int(job["n_samples"])
    if job.get("mixer") is None:
        # Refuse rather than invent. The P0 mono job does not even carry
        # track_id on its notes (routes.js adds it only when chained), so
        # "separate the tracks" has no answer here -- there is one bus. The
        # caller's fix is to send the mixer payload; a DEFAULT mixer is a
        # no-op chain, so that changes nothing audible.
        raise ValueError(
            "render_stems needs the rack path: a job without `mixer` carries "
            "no track_id on its notes, so there are no per-track buses to "
            "write. Send the mixer payload (a default mixer is a no-op chain).")
    out_dir = job.get("out_dir")
    if not out_dir:
        raise ValueError("render_stems needs `out_dir` -- where the stems land.")
    prefix = str(job.get("prefix") or "")
    os.makedirs(out_dir, exist_ok=True)

    mastered, buses = chain_graph(job, synths, capture=True)
    total = mastered.shape[1]
    sl = slice(w0, w0 + n)
    only = job.get("tracks")
    only = {str(t) for t in only} if only else None

    rows = []
    return_rows = []
    exported_sum = np.zeros((2, n), dtype=np.float64)
    for tid in sorted(buses["tracks"]):
        if only is not None and tid not in only:
            continue
        win = buses["tracks"][tid][:, sl].astype(np.float32)
        exported_sum += win
        name = f"{prefix}trk_{tid}.wav"
        dest = os.path.join(out_dir, name)
        tmp = dest + f".tmp-{os.getpid()}"
        write_wav_f32_stereo(tmp, win, sr)
        os.replace(tmp, dest)                    # land atomically, like regions
        inter = np.empty(n * 2, dtype="<f4")
        inter[0::2] = win[0]
        inter[1::2] = win[1]
        rows.append({
            "track_id": tid, "file": name,
            "peak": round(float(np.max(np.abs(win))) if n else 0.0, 6),
            "rms_db": round(_bus_db(win), 2),
            "sha1": hashlib.sha1(inter.tobytes()).hexdigest(),
            "bytes": 44 + n * 8,
        })

    # Shared send effects belong to their own files. Track-only stems omit
    # these buses and cannot reconstruct a mix that uses a hall/delay return.
    for rid, bus in buses["returns"].items():
        win = bus[:, sl].astype(np.float32)
        exported_sum += win
        name = f"{prefix}ret_{rid}.wav"
        dest = os.path.join(out_dir, name)
        tmp = dest + f".tmp-{os.getpid()}"
        write_wav_f32_stereo(tmp, win, sr)
        os.replace(tmp, dest)
        inter = np.ascontiguousarray(win.T, dtype="<f4")
        return_rows.append({
            "return_id": rid, "file": name,
            "peak": round(float(np.max(np.abs(win))) if n else 0.0, 6),
            "rms_db": round(_bus_db(win), 2),
            "sha1": hashlib.sha1(inter.tobytes()).hexdigest(),
            "bytes": 44 + n * 8,
    ...
    # The two honesty numbers, measured over the whole window.
    summed = _sum_buses(buses, total)
    pre = buses["mix"]
    residual = _err_to_sig_db(summed[:, sl] - pre[:, sl], pre[:, sl])
    master_delta = _err_to_sig_db(mastered[:, sl] - pre[:, sl], pre[:, sl])
    exported_residual = _err_to_sig_db(exported_sum - pre[:, sl], pre[:, sl])
    complete = only is None or set(buses["tracks"]).issubset(only)
    tcfg = job.get("mixer", {}).get("tracks") or {}
    silent = sorted(t for t in tcfg if t not in buses["tracks"])
    return {
        "ok": True, "sr": sr, "start_sample": w0, "n_samples": n,
        "channels": 2, "stereo": stereo_on(job.get("mixer")),
        ...
        # -inf here is the good answer: the lanes ARE the mix, bit for bit.
        "residual_db": None if residual == float("-inf") else round(residual, 2),
        "sums_to_mix": complete and residual == float("-inf"),
        "exported_complete": complete,
        "exported_residual_db": None if exported_residual == float("-inf") else round(exported_residual, 2),
        "master_delta_db": None if master_delta == float("-inf") else round(master_delta, 2),
        "sums_to": "Tracks PLUS the separate shared-return files reconstruct the PRE-MASTER mix "
                   "up to float32 export rounding when all tracks are included. Return files "
                   "contain the whole mix's sends even when selecting only some tracks. "
                   "The master chain, the master fader and "
                   "the tanh curve sit on top of this sum, so the lanes do not "
                   "add up to what you hear at the limiter.",
        "ms": round((time.perf_counter() - t_start) * 1000, 1),
    }


def _bus_db(x, floor=1e-12):
    p = float(np.mean(np.square(np.asarray(x, dtype=np.float64))))
    return 10.0 * math.log10(max(p, floor))
...
def lr_correlation(x):
    """Pearson correlation of L and R over the window, rounded; None when
    either channel is silent (a correlation of nothing is not 1.0)."""
    ...
    if den < 1e-18:
        return None
    return round(float(np.sum(l * r) / den), 4)
...
def _bus_meters(x, sr, with_tp=False):
    ...
    row = {
        ...
        # L/R correlation: 1.0 is mono to the byte, ~0.7 is a spread lead.
        # The number the stereo switch exists to move; None when silent.
        "corr": lr_correlation(x),
    }
    if with_tp:
