...
export async function listPatches() {
  ...
  for (const [pid, row] of Object.entries(PATCHES)) {
    ...
    out.push({
      ...
      /* Named knob settings a row may carry ({name: {doc, params}}) --
       * pure data, measured when written. Nothing applies one: a caller
       * sends the values as params. hybrid_kick's `bigroom` is the first. */
