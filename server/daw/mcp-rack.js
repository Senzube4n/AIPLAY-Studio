...

    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          solo: { type: "boolean" },
          stereo: { type: "boolean", description: "op set, target \"master\" only: the rack's "
            + "stereo switch. true renders each voice's own channels (a unison lead's spread, a "
            + "sampled piano's image) through the inserts; false/absent folds every voice to "
            + "(L+R)/2 before the first insert — the byte-identical past, which is why an "
            + "existing project keeps it off until asked. New projects start on. Flipping it "
            + "re-renders every region." },
          target_lufs: { description: "op set, target \"master\" only: number (-30..-6) or null. "
            + "The integrated loudness the BOUNCE aims at in its second pass (gain into a -1 dBTP "
            + "ceiling, at most 3 dB of limiting, reported honestly as reached or short). null = off, "
            + "the rendered bytes exactly. -8 is a club master, -14 streaming." },
