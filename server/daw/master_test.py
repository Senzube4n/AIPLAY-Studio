...
a number and measure it. The claims:
  ...
  LOUDNESS      the stage lands a synthetic big-room drop on -8 LUFS within
                0.5 LU with under 3 dB of limiting and the -1 dBTP ceiling
                held; a sparse mix is refused honestly (budget spent,
                shortfall named, advice points upstream); the encode path
                without target_lufs writes what it always wrote.
...
# ═══════════════════════ 15. THE LOUDNESS STAGE ════════════════════════════

print("\n  -- the loudness stage: measure, gain, limit inside a budget, re-measure, say so --")


def _drop(kick_amp=0.8, lead_amp=0.45, sub_amp=0.3, secs=8.0, sc_depth=0.85):
    """A synthetic big-room drop at 128: a swept kick on every beat, a
    sidechained sub, a seven-voice panned supersaw on 8ths, through the
    rack's tanh. Not music -- a SHAPE with a known crest, so the stage's
    arithmetic can be checked against a signal whose loudness is in the
    body and whose peaks are in the kick, like the real thing."""
    ...
    kn = int(0.35 * SR)
    tk = np.arange(kn) / SR
    f = 45 + 100 * np.exp(-tk / 0.03)
    kick = np.sin(2 * np.pi * np.cumsum(f) / SR) * np.exp(-tk / 0.12) * kick_amp
    kick[:48] += np.linspace(1, 0, 48) * 0.5
    env = np.ones(n)
    for b in range(int(secs / beat)):
        s0 = int(b * beat * SR)
        e0 = min(s0 + kn, n)
        out[:, s0:e0] += kick[:e0 - s0]
        e1 = min(s0 + int(0.18 * SR), n)
        env[s0:e1] *= 1 - sc_depth * np.exp(-np.arange(e1 - s0) / (0.06 * SR))
    out += (np.sin(2 * np.pi * 43.65 * t) * sub_amp * env)[None, :]
    saw = np.zeros((2, n))
    rng = np.random.default_rng(3)
    for v in range(7):
        det, pan = (v - 3) / 3 * 18, (v - 3) / 3 * 0.8
        y_ = 2 * ((t * 349.23 * 2 ** (det / 1200) + rng.uniform(0, 1)) % 1.0) - 1
        saw[0] += y_ * math.sqrt(0.5 * (1 - pan))
        saw[1] += y_ * math.sqrt(0.5 * (1 + pan))
    saw /= math.sqrt(7)
    gate = np.zeros(n)
    for k in range(int(secs / (beat / 2))):
        s0 = int(k * beat / 2 * SR)
        gate[s0:min(s0 + int(beat * 0.45 * SR), n)] = 1.0
    kk = math.exp(-2 * math.pi * 6000 / SR)
    saw = np.vstack([lfilter([1 - kk], [1, -kk], saw[0]), lfilter([1 - kk], [1, -kk], saw[1])])
    out += saw * (gate * env * lead_amp)[None, :]
    return np.tanh(0.7 * out)


y_drop = _drop()
li0, tp0 = rack.lufs_integrated(y_drop, SR), rack.true_peak_db(y_drop)
ok(f"the synthetic drop measures {li0:.2f} LUFS / {tp0:.2f} dBTP, crest {tp0 - li0:.1f} dB "
   "(true peak over LUFS): a big-room shape, too quiet", -16 < li0 < -10 and 7 < tp0 - li0 < 10)
z, rep_ = master.loudness_stage(y_drop, SR, -8.0)
ok(f"target -8 LUFS: lands at {rep_['after']['lufs']} (within 0.5 LU), reached True",
   rep_["reached"] is True and abs(rep_["after"]["lufs"] + 8.0) <= 0.5, json.dumps(rep_))
ok(f"...with {rep_['limiter_work_db']:.2f} dB of limiting, inside the 3 dB budget",
   0.0 < rep_["limiter_work_db"] <= 3.0 + 0.02)
ok(f"...and the true peak {rep_['after']['true_peak_db']} dBTP holds the -1 dBTP ceiling",
   rep_["after"]["true_peak_db"] <= -1.0 + 0.02)
_lp = rack.Params(rack.CATALOG["limiter"], {"ceiling_db": -1.0, "release_ms": 80.0,
                                              "lookahead_ms": 5.0}, SR)
_zz = y_drop * rack.db_to_lin(rep_["gain_db"])
_zz = _zz * rack.limiter_gain(_zz, _lp, ctx())
ok("the reported gain is the gain that was RENDERED: gain then the rack's limiter reproduces "
   "the output to float dust", float(np.max(np.abs(_zz - z))) < 1e-9)
ok("re-measuring the output with the meter agrees with the report",
   abs(rack.lufs_integrated(z, SR) - rep_["after"]["lufs"]) < 0.01)
ok("the stage is deterministic (same buffer, same bytes)",
   np.array_equal(z, master.loudness_stage(y_drop, SR, -8.0)[0]))
ok(f"it took {rep_['rounds']} rounds, and the report says so and carries the plan it started from",
   1 <= rep_["rounds"] <= 4 and rep_["plan"]["reachable"] is True
   and rep_["gain_db"] >= rep_["plan"]["gain_db"] - 1e-9)
y_sp = _drop(kick_amp=0.9, lead_amp=0.06, sub_amp=0.08)
z2, rep2 = master.loudness_stage(y_sp, SR, -8.0)
ok(f"a sparse mix (crest {rep2['plan']['crest_tp_db']} dB against the "
   f"{rep2['plan']['crest_allowed_db']} dB the target allows) does NOT reach -8: "
   f"reached False, {rep2['shortfall_db']} LU short, said so",
   rep2["reached"] is False and rep2["shortfall_db"] > 0.5
   and rep2["plan"]["reachable"] is False, json.dumps(rep2))
ok(f"...the limiter did its whole budget and not a dB more ({rep2['limiter_work_db']:.3f} dB)",
   2.95 <= rep2["limiter_work_db"] <= 3.02)
ok("...the ceiling still holds", rep2["after"]["true_peak_db"] <= -1.0 + 0.02)
ok("...and the advice names the crest against the allowance and points upstream, not at the master",
   "crest" in rep2["advice"] and "upstream" in rep2["advice"])
_r6 = master.loudness_stage(y_sp, SR, -8.0, max_limit_db=6.0)[1]
ok(f"the budget is a knob, and it is spent exactly: max_limit_db 6 does {_r6['limiter_work_db']:.2f} dB "
   f"and gets closer ({_r6['after']['lufs']} vs {rep2['after']['lufs']} LUFS)",
   5.95 <= _r6["limiter_work_db"] <= 6.02 and _r6["after"]["lufs"] > rep2["after"]["lufs"] + 0.5)
ok("...but a kick-dominated mix's loudness dies under limiting: even 6 dB does not buy -8, and "
   "the stage says so instead of pretending", _r6["reached"] is False and _r6["shortfall_db"] > 0.5)
pl = master.plan_loudness(-20.0, -6.0, -8.0)
ok("plan arithmetic: -20 LUFS / -6 dBTP aimed at -8 wants +12, the budget allows +8: "
   "3 dB of limiting, 4 dB short, not reachable",
   pl["gain_db"] == 8.0 and pl["limiting_db"] == 3.0 and pl["shortfall_db"] == 4.0
   and pl["reachable"] is False and pl["expected_lufs"] == -12.0)
pl2 = master.plan_loudness(-12.0, -6.0, -8.0)
ok("...and -12 / -6 aimed at -8 is +4, landing at -2 dBTP: no limiting, reachable",
   pl2["gain_db"] == 4.0 and pl2["limiting_db"] == 0.0 and pl2["reachable"] is True)
pl4 = master.plan_loudness(-12.0, -3.0, -8.0)
ok("...and -12 / -3 aimed at -8 is +4 with exactly 2 dB of limiting",
   pl4["gain_db"] == 4.0 and pl4["limiting_db"] == 2.0 and pl4["reachable"] is True)
pl3 = master.plan_loudness(-6.0, -3.0, -8.0)
ok("...a mix already too loud is turned DOWN and needs no limiting",
   pl3["gain_db"] == -2.0 and pl3["limiting_db"] == 0.0 and pl3["reachable"] is True)
zs, reps = master.loudness_stage(np.zeros((2, SR)), SR, -8.0)
ok("silence is refused honestly (applied False, reached False, bytes untouched)",
   reps["applied"] is False and reps["reached"] is False and np.array_equal(zs, np.zeros((2, SR))))

print("\n  -- the encode path: off is what it always wrote; on aims, measures, reports --")
import instruments as I  # noqa: E402
import soundfile as sf   # noqa: E402
with tempfile.TemporaryDirectory() as td:
    half = y_drop.shape[1] // 2
    parts = []
    for i in range(2):
        pth = os.path.join(td, f"reg{i}.wav")
        rack.write_wav_f32_stereo(pth, y_drop[:, i * half:(i + 1) * half].astype(np.float32), SR)
        parts.append(pth)
    off = I._cli_encode({"sr": SR, "wav_parts": parts, "out": os.path.join(td, "off.flac")})
    ref = np.concatenate([sf.read(p_, dtype="float32", always_2d=True)[0] for p_ in parts])
    ref_path = os.path.join(td, "ref.flac")
    sf.write(ref_path, ref, SR, subtype="PCM_24")
    back_off = sf.read(off["out"], dtype="float64", always_2d=True)[0]
    ok("encode without target_lufs writes the samples it always wrote, and no `loudness` key",
       "loudness" not in off and off["bit_depth"] == 24
       and np.array_equal(back_off, sf.read(ref_path, dtype="float64", always_2d=True)[0]))
    on = I._cli_encode({"sr": SR, "wav_parts": parts, "out": os.path.join(td, "on.flac"),
                        "target_lufs": -8})
    back = sf.read(on["out"], dtype="float64", always_2d=True)[0].T
    lb = rack.lufs_integrated(back, SR)
    ok(f"encode with target_lufs -8 writes a file that MEASURES {lb:.2f} LUFS "
       f"(the reply said {on['loudness']['after']['lufs']})",
       on["loudness"]["reached"] is True and abs(lb + 8.0) <= 0.5
       and abs(lb - on["loudness"]["after"]["lufs"]) < 0.05, json.dumps(on.get("loudness")))
    ok("...24-bit stereo, undithered, and the true peak still holds -1 dBTP after the PCM write",
       on["bit_depth"] == 24 and on["channels"] == 2 and on["dithered"] is False
       and rack.true_peak_db(back) <= -1.0 + 0.03)
    on16 = I._cli_encode({"sr": SR, "wav_parts": parts, "out": os.path.join(td, "on16.flac"),
                          "target_lufs": -8, "bit_depth": 16})
    ok("...with bit_depth 16 the stage runs BEFORE the dither (dithered True, loudness reported)",
       on16["dithered"] is True and on16["loudness"]["reached"] is True)
    mono_part = os.path.join(td, "mono.wav")
    engine.write_wav_f32(mono_part, y_drop[0].astype(np.float32), SR)
    mo = I._cli_encode({"sr": SR, "wav_parts": [mono_part], "out": os.path.join(td, "mo.flac"),
                        "target_lufs": -8})
    ok("a mono (P0-path) bounce goes through as dual mono and comes back MONO, aimed",
       mo["channels"] == 1 and mo["loudness"]["applied"] is True
       and sf.read(mo["out"], always_2d=True)[0].shape[1] == 1)
    try:
        I._cli_encode({"sr": 44100, "wav_parts": parts, "out": os.path.join(td, "x.flac"),
                       "target_lufs": -8})
        ok("a non-48k bounce with a target is refused (the K table is pinned)", False)
    except ValueError as exc:
        ok("a non-48k bounce with a target is refused (the K table is pinned)", "48 kHz" in str(exc))
    ok("every encode reply is JSON-serialisable", isinstance(json.dumps(on), str))

cd2 = master.check_delivery({"sr": SR, "file": mine_path, "target_lufs": -8}, None)
rowc = [r_ for r_ in cd2["results"] if r_["id"] == "custom"]
ok("check_delivery grows a `custom` row for the job's target_lufs, judged by the stage's own plan",
   len(rowc) == 1 and rowc[0]["stage"] == master.plan_loudness(
       cd2["measured"]["lufs"], cd2["measured"]["true_peak_db"], -8.0)
   and rowc[0]["verdict"] in ("pass", "reachable", "needs a denser mix"))
ok("...every platform row carries `stage` too (the same arithmetic, that platform's ceiling)",
   all("stage" in r_ and r_["stage"]["gain_db"] <= r_["gain_change_db"] + 1e-9
       for r_ in cd2["results"] if r_["id"] != "custom"))
ok("...and a `targets` filter without 'custom' leaves the custom row out",
   not [r_ for r_ in master.check_delivery({"sr": SR, "file": mine_path, "target_lufs": -8,
                                            "targets": ["spotify"]}, None)["results"]
        if r_["id"] == "custom"])
