...
PROFILES = {
    ...
    # ⚠ PIANO IS REASONED, NOT MEASURED, unlike the two above.
    #
    # The piano RIG has existed in mcp-vfx.js all along (keys that light on
    # their own pitches, plus a falling piano-roll lane) with no profile to
    # feed it — so a piano had to be transcribed as a guitar, which hears
    # 60-2000 Hz. A piano is 27.5 Hz (A0) to 4186 Hz (C8): the guitar profile
    # cannot hear the bottom octave and a half at all, and loses everything
    # above roughly C7.
    #
    # fmin/fmax are therefore the instrument's real range and are not a
    # judgement call. The other two are:
    #   min_note_ms 45   between guitar's 40 and bass's 60. A sustained ballad
    #                    does not need short notes, but a grace note or a
    #                    quick ornament is ~50 ms and should survive.
    #   vel_gate 0.15    NOT guitar's 0.40. The same reasoning the bass profile
    #                    records — a quiet stem gets starved by a high gate —
    #                    applies harder to a felt-muted piano played softly,
    #                    which is the whole point of the instrument. Not zero,
    #                    because a piano's sustain pedal leaves ringing tails
    #                    that a zero gate would read as new notes.
    #
    # FIRST MEASUREMENT, on one real song rather than a synthetic sweep.
    # "Felt Hammers" (a deliberately soft, felt-muted solo piano ballad),
    # htdemucs `other` stem, same audio through both profiles:
    #
    #     piano    693 notes   A#1 - C6
    #     guitar   526 notes   G2  - C6
    #
    # 32% more notes, and the guitar profile lost the bottom of the left hand
    # outright (G2 floor against A#1). The check that the extra notes are real
    # rather than noise: both transcriptions are 92% coherent with a single key
    # (Bb major / D minor). A gate letting garbage through would have DILUTED
    # that; it did not move at all, so the 167 extra notes are as in-key as the
    # ones both profiles agree on.
    #
    # ⚠ That is evidence, not an F1. One song, no ground-truth MIDI, and the
    # range extension itself barely mattered here — only 3 notes fell below the
    # guitar profile's 60 Hz floor, because this song is played in the middle of
    # the keyboard on purpose. The gain is almost entirely the 0.15 gate. A loud
    # piece with real bass would exercise fmin properly and has not been tried.
    "piano":  dict(min_note_ms=45.0, fmin=27.5, fmax=4186.0, vel_gate=0.15),
}
