...
def ms_per_frame(variants, reps=8, rounds=7):
    ...
    `variants` is [(key, doc, wired)]; returns ({key: best ms}, {key: [ms per
    round]}). The per-round column costs nothing to keep and a DIFFERENCE between
    two variants wants it: two best-of-N minima can come from different rounds,
    and subtracting those is subtracting two unrelated slices of the box's
    afternoon.
    """
    for _key, doc, wired in variants:                   # warm caches, compile
        with contextlib.nullcontext() if wired else unwired():
            render(doc, 0.0)
    best, every = {}, {key: [] for key, _doc, _wired in variants}
...
# The 1080p delta again, round by round instead of as a difference of two
# best-of-N summaries — no extra renders, these ARE the rounds above. It prints
# because the spread of this one column is the whole argument for how the last
# two fences below are written: it is the noise floor of a difference of two
# ~9ms timings, and it is wider than the ~280us it is trying to measure.
big_rounds = rounds_by_label["1080p"]
paired_us = sorted((s - n) * 1000.0
                   for s, n in zip(big_rounds["some"], big_rounds["none"]))
print(f"  MEASURE    1080p  six expressions, per round ... "
      f"{paired_us[0]:+8.1f} /{paired_us[len(paired_us) // 2]:+8.1f} /"
      f"{paired_us[-1]:+8.1f} us   min / median / max of {len(paired_us)}")
...
# This fence used to read (big_some - big_none) < big_none * 0.25, and a quarter
# was already the second guess at a threshold that would hold. It still failed:
# a pre-commit run rejected a commit at +2541.5us against a 2490us fence, and
# the same suite passed minutes later with nothing changed and nothing to show
# for the rejection. Widening it would have been the third guess, and the
# threshold was never the problem. At 1080p the quantity is not measurable. What
# expressions cost is a FIXED amount per frame — ~280us, the line above, and the
# same ~280us at 1080p, because none of this work is per-pixel — and at 1080p
# that signal is a difference between two ~9ms timings. The noise floor of that
# difference beats it even on an idle box: this suite regularly prints a 1080p
# "no expressions, wired" that is FASTER than unwired (-328.4 us in the run this
# comment was written against), which is a cost that cannot exist.
#
# So the claim stays and the arithmetic moves to where it can be read: the
# per-frame cost, measured on the canvas where a frame is cheap enough to see
# it, against the cost of a 1080p frame. "A rounding error against a 1080p
# frame, not a second render" is exactly what that says — with one noisy term
# instead of two, and about 8x of margin instead of none.
eq("...and under a quarter of a 1080p frame",
   (small_some - small_none) < big_none * 0.25, True)
# What the small canvas cannot see is expressions turning into per-PIXEL work,
# so that one regression keeps a fence at 1080p. 36x the pixels would put the
# delta near +100% of a frame; this catches it at half of that, which is far
# outside the spread printed above. Read off the round-by-round minimum, because
# a real regression is in every round and machine load is not.
eq("...and did not become per-pixel work at 1080p",
   paired_us[0] / 1000.0 < big_none * 0.5, True)
