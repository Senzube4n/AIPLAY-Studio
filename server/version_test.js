...
test("a fork names the original it came from; the original carries no lineage block", () => {
  ...
  assert.deepEqual(l && { letter: l.letter, up: l.upstream.repo }, { letter: "B", up: "Senzube4n/AIPLAY-Studio" },
    "this repository is the fork, and says which repository it forked");
  ...
  const v = appVersion();
  assert.ok(v.base || v.sameAsUpstream, "either a base line or 'the same commit'");
...
test("Collab: the protocol decides, the build is a caption", async () => {
  ...
  assert.match(src("../CLAUDE.md"), /node scripts\/stamp-lineage\.mjs/, "the rule that a pull is not finished until it has run");
});
