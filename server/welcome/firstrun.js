 ...
 *      it does not, WHICH half falls short (the card or the RAM), then the ways
 *      that cost nothing: a friend's card first (Collab), then Reactive's cut
 *      styles. The owner's order, 2026-09-24: lending by a friend before any
...
/* 2a. The music model: the one the Music screen runs, so said plainly. */
function musicSentence(pick, cap, notes, caps) {
  ...
  return `Music on ${name}${getText(pick, cap)}.`;
}
...
function videoShortSentence(row, gpu) {
  ...
  if (vramShort && ramShort) {
    return `Video clips: ${name} needs a ${f.needVramGb} GB card and ${f.needRamGb} GB of RAM; this PC has ${f.yourVramGb} GB and ${f.yourRamGb} GB.`;
  }
  if (vramShort) return `Video clips: ${name} needs a ${f.needVramGb} GB card and this one has ${f.yourVramGb} GB.`;
  if (ramShort) {
...
/* The ways that cost nothing, friend first. Lending is built and has passed
 * its tests on one machine; a render between two PCs has not been tried, and
 * the sentence says so rather than promising it (docs/COLLAB.md). Reactive's
 * cut styles run on the compositor alone (catalogue.js, the Reactive card). */
const FREE_WAYS = "A friend's card can be asked to render them in Collab (built, not yet tried between two PCs), "
  + "and Reactive's cut styles run on any card.";
...
export function firstRunLines(models) {
  ...
  const two = [];
  if (music) two.push(musicSentence(music, caps.get(music.id), notes, caps));
  ...
  if (!m.unavailable && !config.musicOnly && !config.cloudOnly) {
    ...
    if (video) {
      ...
      if (row?.fit) {
        lines.push(`${videoShortSentence(row, gpu)} ${FREE_WAYS}`);
        links.push({ label: "Collab", view: "collab" }, { label: "Reactive", view: "reactive" });
      }
    }
  }

  links.push({ label: "Models", view: "models" }, { label: "Settings", view: "settings" });
  return {
    lines,
    links,
