...
const helper=fileURLToPath(new URL('./attachment_fit.py',import.meta.url)),marker='AVATAR_FITTING_RESULT_JSON:';
...
export function runFittingPython(python,args,{timeoutMs=FITTING_LIMITS.timeoutMs,spawnImpl=spawn}={}){
  return new Promise((resolve,reject)=>{
    ...
    try{
      child=spawnImpl(python,['-u',helper,...args],{shell:false,windowsHide:true,stdio:['ignore','pipe','pipe'],env:{...process.env,PYTHONDONTWRITEBYTECODE:'1',HF_HUB_OFFLINE:'1'}});
...
export function createAvatarFittingService({directory,inspectAsset,python,run=runFittingPython,record=async()=>{}}={}){
  ...
  async function interpreter(){const candidates=python||process.env.AIPLAY_AVATAR_PYTHON?[python||process.env.AIPLAY_AVATAR_PYTHON]:[config.mesh.unirigPython,config.mesh.python];
    ...
    throw fault('Configure AIPLAY_AVATAR_PYTHON with local bpy and restart Studio. No packages or models are downloaded automatically.',503);}
