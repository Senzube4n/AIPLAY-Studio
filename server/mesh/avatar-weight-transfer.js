...
const helper=fileURLToPath(new URL('./weight_transfer.py',import.meta.url));
...
export function runWeightPython(python,args,{timeoutMs=WEIGHT_TRANSFER_LIMITS.timeoutMs,spawnImpl=spawn}={}) {
  return new Promise((resolve,reject)=>{
    let child,timer,settled=false,stdout='',stderr='',size=0;
    const finish=(error,value)=>{if(settled)return;settled=true;clearTimeout(timer);error?reject(error):resolve(value);};
    try {
      child=spawnImpl(python,['-u',helper,...args],{shell:false,windowsHide:true,stdio:['ignore','pipe','pipe'],
...
export function createWeightTransferService({directory,python,run=runWeightPython,record=async()=>{},validate=validateOutput,
  ...
  async function interpreter(){
    ...
    for(const candidate of candidates){if(typeof candidate!=='string'||!path.isAbsolute(candidate))continue;try{if((await stat(candidate)).isFile())return candidate;}catch{}}
    throw fault('Local weight transfer needs Blender Python with bpy. Set AIPLAY_AVATAR_PYTHON and restart Studio; nothing is downloaded automatically.',503);
  }
