...
console.log("\n§2  every list, and the door's refusals");
{
  ...
  ok("the config door lists krea2 and gates it on the row", /\[[^\]\n]*"krea2"[^\]\n]*\]\.includes\(b\.engine\)/.test(index) && /if \(b\.engine === "krea2"\) \{\n\s+const cap = \(await models\.status\(\)\)\.find\(\(c\) => c\.id === "imageKrea2"\);/.test(index));
  const createAt = index.indexOf('if (p === "/api/image" && req.method === "POST")');
  const createEnd = index.indexOf('if (p === "/api/', createAt + 20);
  const create = createAt >= 0 && createEnd > createAt ? index.slice(createAt, createEnd) : "";
  ok("the create door lists it", /\[[^\]\n]*"krea2"[^\]\n]*\]\.includes\(engine\)/.test(create));
  ...
  const makeAt = mcp.indexOf('name: "make_image"');
  const makeTool = makeAt >= 0 ? mcp.slice(makeAt, mcp.indexOf('\n    name:', makeAt + 1)) : "";
  ok("the tool's enum names it", /engine: \{ type: "string", enum: \[[^\]\n]*"krea2"[^\]\n]*\]/.test(makeTool));
