 ...
 * Python or a model.
 *
 * Which repository: install-info.json (written by the installer), else the
 * fork named in package.json's lineage, else the original. The same order a
 * person would guess, written down.
 *
...
export async function updateSource(root = ROOT_DEFAULT) {
  if (existsSync(path.join(root, ".git"))) return { kind: "git" };
 ...
 */
export async function selfUpdate({ root = ROOT_DEFAULT, say = () => {} } = {}) {
  const src = await updateSource(root);

  if (src.kind === "git") {
    say("Checking for changes of your own…");
    if ((await run("git", ["status", "--porcelain", "--untracked-files=no"], { cwd: root })).length) {
    ...
    try { await run("git", ["pull", "--ff-only"], { cwd: root, timeout: 120_000 }); }
    catch (e) {
    ...
    if (after === before) return { ok: true, changed: false, line: "Already up to date." };
    await npmIfChanged(root, lockBefore, say);
    return { ok: true, changed: true, restart: true, line: `Updated ${before.slice(0, 7)} → ${after.slice(0, 7)}. Close the launcher and open it again to use the new version.` };
  }

  say(`Asking GitHub for the newest ${src.repo}…`);
  let head;
  try {
    const r = await fetch(`https://api.github.com/repos/${src.repo}/commits/main`, { headers: HEADERS, signal: AbortSignal.timeout(15_000) });
  ...
  if (src.have && sha.startsWith(src.have)) return { ok: true, changed: false, line: "Already up to date." };

  const work = await mkdtemp(path.join(tmpdir(), "aiplay-update-"));
  try {
    say(`Downloading ${sha.slice(0, 7)}…`);
    const zip = path.join(work, "studio.zip");
    const res = await fetch(`https://codeload.github.com/${src.repo}/zip/${sha}`, { headers: { "User-Agent": HEADERS["User-Agent"] } });
    ...
    for (const name of include) {
      ...
      if (info.isDirectory()) {
        await prune(from, exclude, keep);
        /* A whole folder swaps, so a file the new build deleted is gone too
         * rather than left behind to be imported by mistake. The old folder is
         * moved aside first and only removed once the new one is in. */
        const old = `${to}.old-${stamp}`;
        if (existsSync(to)) await rename(to, old);
        try {
        ...
        } catch (e) { await rm(to, { recursive: true, force: true }); if (existsSync(old)) await rename(old, to); throw e; }
        await rm(old, { recursive: true, force: true }).catch(() => {});
      } else {
        /* A running exe cannot be overwritten but can be renamed: the launcher
         * host (AIPLAY Studio.exe) is usually running while this happens. */
        if (existsSync(to)) {
          try { await cp(from, to, { force: true }); }
          catch { const old = `${to}.old-${stamp}`; await rename(to, old); await cp(from, to); }
        } else await cp(from, to);
      }
    }

    await writeFile(path.join(root, "server", "version.gen.json"), JSON.stringify({
      commit: sha.slice(0, 7), date, modified: false, base: null, at: new Date().toISOString(), via: "launcher update", repo: src.repo,
    }));
    const info = await readJson(path.join(root, "install-info.json"));
    if (info) await writeFile(path.join(root, "install-info.json"), JSON.stringify({ ...info, commit: sha, date, updatedAt: new Date().toISOString() }));
    await npmIfChanged(root, lockBefore, say);
    return { ok: true, changed: true, restart: true, line: `Updated to ${sha.slice(0, 7)} from ${src.repo}. Close the launcher and open it again to use the new version.` };
  } catch (e) {
    return { ok: false, changed: false, line: `The update stopped: ${e.message}` };
  } finally {
...
async function npmIfChanged(root, lockBefore, say) {
  const lockAfter = await readFile(path.join(root, "package-lock.json"), "utf8").catch(() => "");
  if (lockAfter === lockBefore) return;
  say("Updating Studio's npm packages…");
  const cli = path.join(path.dirname(process.execPath), "node_modules", "npm", "bin", "npm-cli.js");
  if (existsSync(cli)) await run(process.execPath, [cli, "install", "--omit=dev", "--no-audit", "--no-fund"], { cwd: root, timeout: 600_000 }).catch(() => {});
  else await run(process.platform === "win32" ? "npm.cmd" : "npm", ["install", "--omit=dev", "--no-audit", "--no-fund"], { cwd: root, timeout: 600_000, shell: process.platform === "win32" }).catch(() => {});
}
