...
export async function selfUpdate({ root = ROOT_DEFAULT, say = () => {}, command = run } = {}) {
  ...
  if (!/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(src.repo)) return { ok: false, changed: false, line: "This install has an invalid update repository." };
  ...
  if (!/^[0-9a-f]{7,40}$/i.test(sha)) return { ok: false, changed: false, line: "GitHub returned an invalid commit." };
  ...
  try {
    ...
    const res = await fetch(`https://codeload.github.com/${src.repo}/zip/${sha}`, { headers: { "User-Agent": HEADERS["User-Agent"] }, signal: AbortSignal.timeout(600_000) });
    ...
    const info = await readJson(path.join(root, "install-info.json"));
    if (info) {
      await writeFile(path.join(fresh, "install-info.json"), JSON.stringify({ ...info, commit: sha, date, updatedAt: new Date().toISOString() }));
      include.push("install-info.json");
    }
    await replaceAppFiles({ root, fresh, include, exclude, keep, preserve });
    return dependencies(lockBefore, true, `Updated to ${sha.slice(0, 7)} from ${src.repo}. Close the launcher and open it again to use the new version.`);
  } catch (e) {
