...
test("routes: another website cannot save a key or spend credits; file names cannot escape", async () => {
  ...
  const routes = createRouterRoutes({
    json, readBody: async () => ({ action: "key", key: "x".repeat(30) }), config: { uiPort: 4173, cloudOnly: true },
  ...
  await routes({ method: "GET", headers: {} }, {}, new URL("http://x/api/router/file/..%2F..%2Fsecrets.json"));
  assert.equal(out.pop().status, 400);
